...
use kumandra_networking::{BootstrappedNetworkingParameters, Config, RelayMode};
...
async fn main() {
    ...
    let (relay_node, mut relay_node_runner) = kumandra_networking::create(config_1).await.unwrap();
    ...
    let (node, mut node_runner) = kumandra_networking::create(config).await.unwrap();

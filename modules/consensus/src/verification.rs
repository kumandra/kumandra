...
//! Verification for Kumandra headers.
use super::{kumandra_err, BlockT, Error};
use log::{debug, trace};
...
use kumandra_archiving::archiver;
use kumandra_core_primitives::{PieceIndex, Randomness, Salt, Sha256Hash, Solution};
use kumandra_solving::{
    derive_global_challenge, is_local_challenge_valid, PieceDistance, KumandraCodec,
};
...
pub(super) fn check_header<B: BlockT + Sized>(
    ...
    trace!(target: "kumandra", "Checking header");
    let seal = header
        .digest_mut()
        .pop()
        .ok_or_else(|| kumandra_err(Error::HeaderUnsealed(header.hash())))?;

    let sig = seal
        .as_kumandra_seal()
        .ok_or_else(|| kumandra_err(Error::HeaderBadSeal(header.hash())))?;
    ...
    if !pre_digest.solution.public_key.verify(&pre_hash, &sig) {
        return Err(kumandra_err(Error::BadSignature(pre_hash)));
    }
...
fn check_piece_tag<B: BlockT>(
    ...
    if !kumandra_solving::is_tag_valid(&solution.encoding, salt, solution.tag) {
...
pub(crate) fn check_piece<B: BlockT>(
    ...
    let kumandra_codec = KumandraCodec::new(&solution.public_key);
    kumandra_codec

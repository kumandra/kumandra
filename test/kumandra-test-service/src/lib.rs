...
use kumandra_service::{FullPool, NewFull, KumandraConfiguration};
use kumandra_test_client::{
...
pub async fn run_validator_node(
    ...
    let primary_chain_node = {
        ...
        kumandra_service::new_full::<kumandra_test_runtime::RuntimeApi, TestExecutorDispatch>(
            primary_chain_config,
            false,
            SlotProportion::new(98f32 / 100f32),
        )
        .await
        .expect("Failed to create Kumandra primary node")
    };
...
/// A Kumandra primary test node instance used for testing.

...
use crate::{
    ArchivedSegmentNotification, ImportedBlockNotification, KumandraLink,
    KumandraNotificationSender,
};
use codec::Encode;
...
use kumandra_core_primitives::{BlockNumber, RootBlock};
...
where
    ...
    Client::Api: KumandraApi<Block, FarmerPublicKey> + ObjectsApi<Block>,
{
    ...
    Client::Api: KumandraApi<Block, FarmerPublicKey> + ObjectsApi<Block>,
{
        ...
        maybe_last_archived_block
    {
        ...
        info!(target: "kumandra", "Starting archiving from genesis");
        ...
        if let Some(blocks_to_archive_to) = blocks_to_archive_to {
            ...
            for block_hash_to_archive in block_hashes_to_archive.into_iter().rev() {
                ...
                if !new_root_blocks.is_empty() {
                    ...
                    kumandra_link.root_blocks.lock().put(
    ...
    Client: LockImportRun<Block, Backend> + Finalizer<Block, Backend>,
{
    ...
    let _result: Result<_, sp_blockchain::Error> = client.lock_import_and_run(|import_op| {
        ...
        client
            ...
            .map_err(|error| {
                warn!(target: "kumandra", "Error applying finality to block {:?}: {}", (hash, number), error);
                error
            })?;

        debug!(target: "kumandra", "Finalizing blocks up to ({:?}, {})", number, hash);

        telemetry!(
            telemetry;
            CONSENSUS_INFO;
            "kumandra.finalized_blocks_up_to";
...
pub fn start_kumandra_archiver<Block, Backend, Client>(
    kumandra_link: &KumandraLink<Block>,
    client: Arc<Client>,
    telemetry: Option<TelemetryHandle>,
    ...
    spawner.spawn_essential_blocking(
        ...
        Box::pin({
            let mut imported_block_notification_stream =
                kumandra_link.imported_block_notification_stream.subscribe();
            let archived_segment_notification_sender =
                kumandra_link.archived_segment_notification_sender.clone();
...
async fn send_archived_segment_notification(
    ...
    let (acknowledgement_sender, mut acknowledgement_receiver) =
        tracing_unbounded("kumandra_acknowledgement");

...
use crate::{
    find_pre_digest, kumandra_err, verification, BlockSigningNotification, NewSlotInfo,
    NewSlotNotification, KumandraLink,
};
...
use kp_consensus::{FarmerPublicKey, KumandraApi};
...
pub(super) struct KumandraSlotWorker<B: BlockT, C, E, I, SO, L, BS> {
    ...
    pub(super) kumandra_link: KumandraLink<B>,
...
impl<B, C, E, I, Error, SO, L, BS> SimpleSlotWorker<B> for KumandraSlotWorker<B, C, E, I, SO, L, BS>
where
    B: BlockT,
    ...
    Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
    ...
    async fn claim_slot(
        ...
        let new_slot_info = NewSlotInfo {
            slot,
            global_challenge: kumandra_solving::derive_global_challenge(&global_randomness, slot),
            salt,
            next_salt,
            solution_range,
        };
        ...
        while let Some(solution) = solution_receiver.next().await {
            ...
            let max_plot_size = runtime_api
                ...
                //  to newer
                .unwrap_or(100 * 1024 * 1024 * 1024 / kumandra_core_primitives::PIECE_SIZE as u64);
            ...
            match verification::verify_solution::<B>(
                ...
                Ok(_) => {
                    debug!(target: "kumandra", "Claimed slot {}", slot);

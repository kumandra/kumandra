...
///
/// Must be called before Kumandra's WASM builder.
pub fn create_runtime_bundle_inclusion_file(
    ...
    let target_dir = env::var("CARGO_TARGET_DIR")
        .or_else(|_| env::var("KUMANDRA_WASM_TARGET_DIRECTORY"))
        .map(Into::into)
        .unwrap_or_else(|_| {
    ...
    if env::var("KUMANDRA_WASM_TARGET_DIRECTORY").is_err() {
        env::set_var("KUMANDRA_WASM_TARGET_DIRECTORY", &target_dir);
    }

    env::set_var(
        "KUMANDRA_WASM_BUNDLE_PATH",
        env::var("KUMANDRA_WASM_BUNDLE_PATH").unwrap_or_else(|_| {
    ...
    let execution_wasm_bundle_path = env::var("KUMANDRA_WASM_BUNDLE_PATH").expect("Set above; qed");

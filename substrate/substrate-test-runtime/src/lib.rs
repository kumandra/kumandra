...
use kc_consensus::{FarmerPublicKey, SignedVote};
...
impl From<pallet_kumandra::Event<Runtime>> for Event {
    fn from(_evt: pallet_kumandra::Event<Runtime>) -> Self {
        unimplemented!("Not required in tests!")
    }
}
...
cfg_if! {
    if #[cfg(feature = "std")] {
        impl_runtime_apis! {
            ...
            impl kc_consensus::KumandraApi<Block, FarmerPublicKey> for Runtime {
                ...
                fn global_randomnesses() -> kc_consensus::GlobalRandomnesses {
                    <pallet_kumandra::Pallet<Runtime>>::global_randomnesses()
                }

                fn solution_ranges() -> kc_consensus::SolutionRanges {
                    <pallet_kumandra::Pallet<Runtime>>::solution_ranges()
                }

                fn salts() -> kc_consensus::Salts {
                    <pallet_kumandra::Pallet<Runtime>>::salts()
                }

                fn submit_report_equivocation_extrinsic(
                    equivocation_proof: kc_consensus::EquivocationProof<
                ...
                fn is_in_block_list(farmer_public_key: &kc_consensus::FarmerPublicKey) -> bool {
                ...
                fn root_plot_public_key() -> Option<FarmerPublicKey> {
                    <pallet_kumandra::Pallet<Runtime>>::root_plot_public_key()
                }
            }

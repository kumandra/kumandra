...
async fn run_executor(
    ...
    let primary_chain_full_node = {
        ...
        .await
        .map_err(|e| {
            sc_service::Error::Other(format!("Failed to build a full kumandra node: {e:?}"))
        })?
    };
...
impl TestNodeBuilder {
    ...
    pub fn connect_to_primary_chain_node(
        mut self,
        node: &kumandra_test_service::PrimaryTestNode,
    ) -> Self {
    ...
    pub fn connect_to_primary_chain_nodes<'a>(
        mut self,
        nodes: impl IntoIterator<Item = &'a kumandra_test_service::PrimaryTestNode>,
    ) -> Self {
        self.primary_nodes
...
) -> (kumandra_test_service::PrimaryTestNode, NetworkStarter) {
    kumandra_test_service::run_validator_node(tokio_handle, key, boot_nodes, true).await
}

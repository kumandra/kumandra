...
use neak_client_executor::{Executor, ExecutorSlotInfo};
use neak_client_executor_gossip::ExecutorGossipParams;
use neak_runtime::{opaque::Block, RuntimeApi};
...
use std::sync::Arc;
use kumandra_core_primitives::Tag;

/// Native executor instance.
pub struct NeakRuntimeExecutor;

impl NativeExecutionDispatch for NeakRuntimeExecutor {
	type ExtendHostFunctions = frame_benchmarking::benchmarking::HostFunctions;

	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		neak_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		neak_runtime::native_version()
	}
}

/// Neak-like full client.
...
) -> sc_service::error::Result<NewFull<Arc<FullClient<RuntimeApi, NeakRuntimeExecutor>>>>
where
	PBlock: BlockT,
	...
	NSNS: Stream<Item = (Slot, Tag)> + Send + 'static,
{
	...
	parachain_config
		.network
		.extra_sets
		.push(neak_client_executor_gossip::executor_gossip_peers_set_config());

...
> {
	...
	pub primary_chain_full_node: kumandra_service::NewFull<RClient>,
...
where
	...
	Client: Finalizer<Block, Backend>
		...
		+ Sync
		+ 'static,
	Client::Api: neak_primitives::SecondaryApi<Block, neak_primitives::AccountId>
	...
	CIDP: CreateInherentDataProviders<Block, neak_primitives::Hash> + 'static,
	E: CodeExecutor,
{
	...
	let executor_gossip = neak_client_executor_gossip::start_gossip_worker(
		neak_client_executor_gossip::ExecutorGossipParams {
			network,
			executor,
			bundle_receiver,
	...
	task_manager
		.spawn_essential_handle()
		.spawn_blocking("neak-gossip", None, executor_gossip);
...
pub struct StartFullNodeParams<'a, Block: BlockT, Client, PClient> {
	pub client: Arc<Client>,
	pub primary_chain_full_node: kumandra_service::NewFull<PClient>,
	pub task_manager: &'a mut TaskManager,
	pub announce_block: Arc<dyn Fn(Block::Hash, Option<Vec<u8>>) + Send + Sync>,
}

...
use kumandra_core_primitives::{BlockNumber, Randomness, Sha256Hash};
	...
	E: CodeExecutor,
{
	...
	fn create_extrinsic_execution_proof(
		...
		prover: &kumandra_fraud_proof::ExecutionProver<Block, Backend, E>,
		...
		loop {
			match aux_schema::load_execution_receipt(&*self.client, secondary_block_hash) {
				...
				Ok(None) => {
					...
					//       ref https://github.com/kumandra/kumandra/pull/250#discussion_r804247551
					//

...
impl RelayChainCli {
	...
	pub fn new<'a>(
		...
		Self { base_path, chain_id, base: kumandra_node::RunCmd::parse_from(relay_chain_args) }
	}
}
...
impl SubstrateCli for RelayChainCli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		<kumandra_node::Cli as SubstrateCli>::from_iter([RelayChainCli::executable_name()].iter())
			.load_spec(id)
	}

	fn native_runtime_version(chain_spec: &Box<dyn ChainSpec>) -> &'static RuntimeVersion {
		kumandra_node::Cli::native_runtime_version(chain_spec)
	}
}

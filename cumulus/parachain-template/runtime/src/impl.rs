...
pub use neak_primitives::{AccountId, Address, Balance, BlockNumber, Hash, Index, Signature};
use frame_support::{
...
pub type Executive = neak_pallet_executive::Executive<
	Runtime,
	Block,
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("kumandra-executor"),
	impl_name: create_runtime_str!("kumandra-executor"),
	authoring_version: 1,
	spec_version: 1,
	impl_version: 0,
...
impl neak_pallet_executive::Config for Runtime {
	type Event = Event;
	type Call = Call;
}
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		ExecutivePallet: neak_pallet_executive = 1,
...
impl_runtime_apis! {
	...
	impl neak_primitives::SecondaryApi<Block, AccountId> for Runtime {
		fn extract_signer(
			extrinsics: Vec<<Block as BlockT>::Extrinsic>,
		) -> Vec<(Option<AccountId>, <Block as BlockT>::Extrinsic)> {
			use neak_primitives::Signer;
		...
		fn construct_set_code_extrinsic(code: Vec<u8>) -> Vec<u8> {
			...
			UncheckedExtrinsic::new_unsigned(
				neak_pallet_executive::Call::sudo_unchecked_weight_unsigned {

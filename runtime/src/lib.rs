...
use primitives::{
    staking::MAX_NOMINATORS_REWARDED_PER_VALIDATOR, wrap_methods, ApiError as KumandraApiError,
    AuthorityId as KumandraId, DEFAULT_MILLISECS_PER_BLOCK, DEFAULT_SESSIONS_PER_ERA,
...
construct_runtime!(
        ...
        UncheckedExtrinsic = UncheckedExtrinsic
    {
        ...
        Kumandra: pallet_kumandra::{Pallet, Storage} = 11,
...
impl_runtime_apis! {
    ...
    impl primitives::KumandraSessionApi<Block> for Runtime {
        fn authorities() -> Vec<KumandraId> {
            Kumandra::authorities()
        }
        ...
        fn next_session_authorities() -> Result<Vec<KumandraId>, KumandraApiError> {
            Session::queued_keys()
                .iter()
                .map(|(_, key)| key.get(KumandraId::ID).ok_or(KumandraApiError::DecodeKey))
                .collect::<Result<Vec<KumandraId>, KumandraApiError>>()
        }
    }

...
pub use pallet_kumandra_staking::StakerStatus;
#[cfg(any(feature = "std", test))]
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("kumandra-node"),
	impl_name: create_runtime_str!("kumandra-node"),
	authoring_version: 1,
...
pub const RRSC_GENESIS_EPOCH_CONFIG: kmp_consensus_rrsc::RRSCEpochConfiguration =
	kmp_consensus_rrsc::RRSCEpochConfiguration {
		c: PRIMARY_PROBABILITY,
		allowed_slots: kmp_consensus_rrsc::AllowedSlots::PrimaryAndSecondaryPlainSlots,
	};
...
parameter_types! {
	...
	pub RuntimeBlockWeights: BlockWeights = BlockWeights::builder()
		...
		.build_or_panic();
	pub const SS58Prefix: u16 = 11333;
}
...
impl pallet_session::Config for Runtime {
	...
	type ValidatorIdOf = pallet_kumandra_staking::StashOf<Self>;
...
impl pallet_session::historical::Config for Runtime {
	type FullIdentification = pallet_kumandra_staking::Exposure<AccountId, Balance>;
	type FullIdentificationOf = pallet_kumandra_staking::ExposureOf<Runtime>;
}
...
impl pallet_kumandra_staking::BenchmarkingConfig for StakingBenchmarkingConfig {
	type MaxNominators = ConstU32<0>;
	type MaxValidators = ConstU32<1000>;
}

impl pallet_kumandra_staking::Config for Runtime {
	...
	type WeightInfo = pallet_kumandra_staking::weights::SubstrateWeight<Runtime>;
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		Staking: pallet_kumandra_staking = 23,
...
impl_runtime_apis! {
	...
	impl kmp_consensus_rrsc::RRSCApi<Block> for Runtime {
		fn configuration() -> kmp_consensus_rrsc::RRSCGenesisConfiguration {
			// The choice of `c` parameter (where `1 - c` represents the
			// probability of a slot being empty), is done in accordance to the
			// slot duration and expected target block time, for safely
			// resisting network delays of maximum two seconds.

			kmp_consensus_rrsc::RRSCGenesisConfiguration {
		...
		fn current_epoch_start() -> kmp_consensus_rrsc::Slot {
			Babe::current_epoch_start()
		}

		fn current_epoch() -> kmp_consensus_rrsc::Epoch {
			Babe::current_epoch()
		}

		fn next_epoch() -> kmp_consensus_rrsc::Epoch {
			Babe::next_epoch()
		}

		fn generate_key_ownership_proof(
			_slot: kmp_consensus_rrsc::Slot,
			authority_id: kmp_consensus_rrsc::AuthorityId,
		) -> Option<kmp_consensus_rrsc::OpaqueKeyOwnershipProof> {
			use codec::Encode;

			Historical::prove((kmp_consensus_rrsc::KEY_TYPE, authority_id))
				.map(|p| p.encode())
				.map(kmp_consensus_rrsc::OpaqueKeyOwnershipProof::new)
		}

		fn submit_report_equivocation_unsigned_extrinsic(
			equivocation_proof: kmp_consensus_rrsc::EquivocationProof<<Block as BlockT>::Header>,
			key_owner_proof: kmp_consensus_rrsc::OpaqueKeyOwnershipProof,
		) -> Option<()> {

...
impl MultiFarming {
    ...
    pub async fn new(
        ...
        for (plot_index, max_plot_pieces) in plot_sizes.into_iter().enumerate() {
            ...
            let (plot, kumandra_codec, plot_commitments, farming) = farm_single_plot(
                base_directory,
                reward_address,
                client.clone(),
                max_plot_pieces,
...
pub(crate) async fn farm_single_plot(
    ...
    client: NodeRpcClient,
    max_plot_pieces: u64,
) -> anyhow::Result<(Plot, KumandraCodec, Commitments, Farming)> {
    ...
    Ok((plot, kumandra_codec, commitments, farming_instance))
}

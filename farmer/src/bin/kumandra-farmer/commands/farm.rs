...
use std::sync::Arc;
use kumandra_farmer::legacy_multi_plots_farm::{
    LegacyMultiPlotsFarm, Options as MultiFarmingOptions,
};
use kumandra_farmer::single_disk_farm::{SingleDiskFarm, SingleDiskFarmOptions};
use kumandra_farmer::single_plot_farm::PlotFactoryOptions;
use kumandra_farmer::ws_rpc_server::{RpcServer, RpcServerImpl};
use kumandra_farmer::{LegacyObjectMappings, NodeRpcClient, Plot, RpcClient};
use kumandra_networking::libp2p::multiaddr::Protocol;
use kumandra_networking::{Config, RelayMode};
use kumandra_rpc_primitives::FarmerProtocolInfo;
use tokio::signal;
...
pub(crate) async fn farm_multi_disk(
    ...
    let (relay_server_node, mut relay_node_runner) = kumandra_networking::create(Config {
        listen_on,

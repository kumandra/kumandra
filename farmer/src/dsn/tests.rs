...
async fn test_dsn_sync() {
    ...
    let seeder_multi_farming = LegacyMultiPlotsFarm::new(
        MultiFarmingOptions {
            ...
            reward_address: kumandra_core_primitives::PublicKey::default(),
...
async fn pieces_by_range_protocol_smoke() {
    ...
    let (node_1, mut node_runner_1) = kumandra_networking::create(config_1).await.unwrap();
    ...
    let (node_2, mut node_runner_2) = kumandra_networking::create(config_2).await.unwrap();
    tokio::spawn(async move {
...
async fn get_pieces_by_range_smoke() {
    ...
    let (mut node_2, mut node_runner_2) = kumandra_networking::create(config_2).await.unwrap();
    tokio::spawn(async move {
        node_runner_2.run().await;
    });

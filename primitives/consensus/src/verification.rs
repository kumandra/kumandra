...
use kumandra_core_primitives::{PieceIndex, Randomness, Salt, Sha256Hash, Solution, Tag, U256};
use kumandra_solving::{
    derive_global_challenge, derive_target, is_tag_valid, verify_local_challenge,
    verify_tag_signature, KumandraCodec,
};
    ...
    Header: HeaderT,
{
    ...
    let kumandra_codec = KumandraCodec::new(solution.public_key.as_ref());
    kumandra_codec
...
pub fn is_within_solution_range(target: Tag, tag: Tag, solution_range: u64) -> bool {
    ...
    kumandra_core_primitives::bidirectional_distance(&target, &tag) <= solution_range / 2
}

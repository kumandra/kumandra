...
//! Primitives for Kumandra consensus.
...
use kumandra_core_primitives::{Randomness, RootBlock, Salt, Sha256Hash};
    ...
    H: HeaderT,
{
    let find_pre_digest = |header: &H| -> Option<PreDigest<FarmerPublicKey>> {
        header
            ...
            .logs()
            .iter()
            .find_map(|log| log.as_kumandra_pre_digest())
    };

    let verify_seal_signature = |mut header: H, offender: &FarmerPublicKey| {
        let seal = CompatibleDigestItem::as_kumandra_seal(&header.digest_mut().pop()?)?;
...
/// Kumandra global randomnesses used for deriving global challenges.
...
/// Kumandra solution ranges used for challenges.
...
sp_api::decl_runtime_apis! {
    /// API necessary for block authorship with Kumandra.
    pub trait KumandraApi {
        ...
        /// The slot duration in milliseconds for Kumandra.

...
pub trait CompatibleDigestItem: Sized {
    ...
    /// If this item is a Kumandra global randomness, return it.
    ...
    /// If this item is a Kumandra next global randomness, return it.
    ...
    /// If this item is a Kumandra next solution range, return it.
    ...
    /// If this item is a Kumandra next salt, return it.
    ...
    /// If this item is a Kumandra records root, return it.
...
impl CompatibleDigestItem for DigestItem {
    ...
    fn next_salt(salt: Salt) -> Self {
        Self::Consensus(KUMANDRA_ENGINE_ID, ConsensusLog::NextSalt(salt).encode())
    }
...
pub enum Error {
    /// Kumandra digest missing
    #[cfg_attr(feature = "thiserror", error("Kumandra {0} digest not found"))]
    Missing(ErrorDigestType),
    /// Failed to decode Kumandra digest
    #[cfg_attr(
        feature = "thiserror",
        error("Failed to decode Kumandra {0} digest: {1}")
    )]
    FailedToDecode(ErrorDigestType, codec::Error),
    /// Duplicate Kumandra digests
    #[cfg_attr(
        feature = "thiserror",
        error("Duplicate Kumandra {0} digests, rejecting!")
    )]
...
pub struct KumandraDigestItems<PublicKey, RewardAddress, Signature> {
    /// Pre-runtime digest
...
/// Extract the Kumandra global randomness from the given header.
pub fn extract_kumandra_digest_items<Header, PublicKey, RewardAddress, Signature>(
    header: &Header,
) -> Result<KumandraDigestItems<PublicKey, RewardAddress, Signature>, Error>
where
    Header: HeaderT,
    PublicKey: Decode,
...
fn derive_next_solution_range<Header: HeaderT>(
    ...
    let next_solution_range = kumandra_verification::derive_next_solution_range(
...
pub struct NextDigestsVerificationParams<'a, Header: HeaderT> {
    ...
    pub header_digests: &'a KumandraDigestItems<FarmerPublicKey, FarmerPublicKey, FarmerSignature>,

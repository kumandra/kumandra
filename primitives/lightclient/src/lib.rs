...
//! Light client substrate primitives for Kumandra.
#![forbid(unsafe_code)]
...
use kp_consensus::digests::{
    extract_pre_digest, extract_kumandra_digest_items, verify_next_digests, CompatibleDigestItem,
...
pub struct HeaderExt<Header> {
    /// Actual header of the kumandra block chain at a specific number.
    pub header: Header,
...
pub enum ImportError<Header: HeaderT> {
    ...
    InvalidSolution(kumandra_verification::Error),
    /// Arithmetic error.
...
impl<Header: HeaderT, Store: Storage<Header>> HeaderImporter<Header, Store> {
    ...
    fn next_salt_derivation_info(
        ...
        let maybe_next_index = kumandra_verification::derive_next_eon_index(
    ...
    fn verify_header_digest_with_parent(
        ...
        KumandraDigestItems<FarmerPublicKey, FarmerPublicKey, FarmerSignature>,
        ImportError<Header>,
    > {
        // extract digest items from the header
        let pre_digest_items = extract_kumandra_digest_items(header)?;
    ...
    fn total_pieces(&self, chain_tip: HashOf<Header>) -> Result<u64, ImportError<Header>> {
        ...
        while header.header.hash() != finalized_header.header.hash() {
            let digest_items = extract_kumandra_digest_items::<
                _,
                FarmerPublicKey,
                FarmerPublicKey,
                FarmerSignature,

...
use kp_consensus::digests::{
    extract_pre_digest, extract_kumandra_digest_items, CompatibleDigestItem, ErrorDigestType,
    PreDigest, KumandraDigestItems,
};
...
use kumandra_solving::{
    ...
    derive_target, KumandraCodec, REWARD_SIGNING_CONTEXT,
};
...
fn derive_solution_range(target: Tag, tag: Tag) -> SolutionRange {
    ...
    kumandra_core_primitives::bidirectional_distance(&target, &tag) * 2
}
...
fn valid_piece(pub_key: schnorrkel::PublicKey) -> (Piece, u64, SegmentIndex, RecordsRoot) {
    ...
    assert!(kumandra_archiving::archiver::is_piece_valid(
        piece,
    ...
    let codec = KumandraCodec::new(pub_key.as_ref());
...
fn valid_header(
    ...
    let mut digests = vec![
        ...
        DigestItem::kumandra_pre_digest(&pre_digest),
    ];

    if should_add_next_randomness {
        let next_global_randomness = derive_randomness(
            &kumandra_core_primitives::PublicKey::from(&FarmerPublicKey::unchecked_from(
    ...
    header
        .digest
        .logs
        .push(DigestItem::kumandra_seal(signature));
...
fn header_import_reorg_at_same_height(new_header_weight: Ordering) {
    ...
    let digests: KumandraDigestItems<FarmerPublicKey, FarmerPublicKey, FarmerSignature> =
        extract_kumandra_digest_items(&header).unwrap();
...
fn test_header_import_success() {
    ...
    let ancestor_digests =
        extract_kumandra_digest_items::<Header, FarmerPublicKey, FarmerPublicKey, FarmerSignature>(
    ...
    let digests_at_11 =
        extract_kumandra_digest_items::<_, FarmerPublicKey, FarmerPublicKey, FarmerSignature>(
            &header_at_11.header,
        )
        .unwrap();
    let randomness = derive_randomness(

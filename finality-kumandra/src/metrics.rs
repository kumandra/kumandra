...
impl<H: Key> Inner<H> {
    fn report_block(&mut self, hash: H, checkpoint_time: Instant, checkpoint_type: Checkpoint) {
        trace!(target: "kumandra-metrics", "Reporting block stage: {:?} (hash: {:?}, at: {:?}", checkpoint_type, hash, checkpoint_time);
        ...
        if let Some(prev_checkpoint_type) = self.prev.get(&checkpoint_type) {
                ...
                .get(&hash)
            {
                let duration = match checkpoint_time.checked_duration_since(*start) {
                    ...
                    None => {
                        warn!(target: "kumandra-metrics", "Earlier metrics time {:?} is later that current one \
...
impl<H: Key> Metrics<H> {
    pub fn register(registry: &Registry) -> Result<Self, PrometheusError> {
        ...
        for key in keys.iter() {
            gauges.insert(
                *key,
                register(Gauge::new(format!("kumandra_{:?}", key), "no help")?, registry)?,
            );
        }

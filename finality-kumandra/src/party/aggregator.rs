use crate::{
    ...
    justification::{KumandraJustification, JustificationNotification},
    metrics::Checkpoint,
    network::DataNetwork,
    party::{AuthoritySubtaskCommon, Task},
    BlockHashNum, Metrics, SessionBoundaries,
};
use kumandra_bft::{KeyBox as BftKeyBox, SignatureSet, SpawnHandle};
use kumandra_bft_rmc::{DoublingDelayScheduler, ReliableMulticast};
use futures::{
...
{
    trace!(target: "kumandra-party", "Received unit {:?} in aggregator.", block);
    ...
    C: HeaderBackend<B> + Send + Sync + 'static,
{
    ...
    if let Err(e) = justifications_for_chain.unbounded_send(notification) {
        error!(target: "kumandra-party", "Issue with sending justification from Aggregator to JustificationHandler {:?}.", e);
    }
}
    ...
    <B as Block>::Hash: AsRef<[u8]>,
{
    ...
    loop {
        trace!(target: "kumandra-party", "Aggregator Loop started a next iteration");
        tokio::select! {
            maybe_block = blocks_from_interpreter.next() => {
                if let Some(block) = maybe_block {
                    ...
                    debug!(target: "kumandra-party", "Blocks ended in aggregator. Terminating.");
                    break;
                }
            }
            multisigned_hash = aggregator.next_multisigned_hash() => {
                if let Some((hash, multisignature)) = multisigned_hash {
                    ...
                    debug!(target: "kumandra-party", "The stream of multisigned hashes has ended. Terminating.");
                    return;
                }
            }
            _ = &mut exit_rx => {
                debug!(target: "kumandra-party", "Aggregator received exit signal. Terminating.");
                return;
            }
        }
    }
    debug!(target: "kumandra-party", "Aggregator awaiting an exit signal.");
    ...
    N: DataNetwork<RmcNetworkData<B>> + 'static,
{
    ...
    let task = {
        async move {
            ...
            debug!(target: "kumandra-party", "Running the aggregator task for {:?}", session_id);
            run_aggregator(
                aggregator_io,
                io,
                client,
            ...
            .await;
            debug!(target: "kumandra-party", "Aggregator task stopped for {:?}", session_id);
        }
    };

    let handle = spawn_handle.spawn_essential("kumandra/consensus_session_aggregator", task);

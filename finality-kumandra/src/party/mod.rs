use crate::{
    ...
    default_kumandra_config,
    justification::{KumandraJustification, JustificationNotification, Verifier},
...
use kumandra_bft::{DelayConfig, SpawnHandle};
use kumandra_primitives::KEY_TYPE;
use codec::Encode;
...
impl<B: Block> Verifier<B> for AuthorityVerifier {
    fn verify(&self, justification: &KumandraJustification, hash: B::Hash) -> bool {
        if !self.is_complete(&hash.encode()[..], &justification.signature) {
            warn!(target: "kumandra-justification", "Bad justification for block hash #{:?} {:?}", hash, justification);
            return false;
        }
        true
    }
...
async fn get_node_index(
    ...
    trace!(target: "kumandra-data-store", "Found {:?} consensus keys in our local keystore {:?}", our_consensus_keys.len(), our_consensus_keys);
    authorities
        .iter()
    ...
    RB: RequestBlocks<B> + 'static,
{
    ...
    async fn spawn_authority_subtasks(
        ...
        let consensus_config = create_kumandra_config(
            authorities.len(),
            node_id,
            session_id,
        ...
        let (unfiltered_kumandra_network, rmc_network) = split(data_network);
        let (data_store, kumandra_network) = DataStore::new(
            session_boundaries.clone(),
            self.client.clone(),
            self.block_requester.clone(),
            Default::default(),
            unfiltered_kumandra_network,
        );
    ...
    async fn spawn_authority_task(
        ...
        AuthorityTask::new(
            self.spawn_handle
                .spawn_essential("kumandra/session_authority", async move {
                    if authority_subtasks.failed().await {
                        warn!(target: "kumandra-party", "Authority subtasks failed.");
                    }
                }),
            node_id,
            exit,
    ...
    async fn run_session(&mut self, session_id: SessionId) {
        ...
        if self.client.info().best_number >= last_block {
            ...
            for attempt in 0..10 {
                ...
                if last_finalized_number >= last_block {
                    debug!(target: "kumandra-party", "Skipping session {:?} early because block {:?} is already finalized", session_id, last_finalized_number);
                    return;
                }
            }
        }
        ...
        {
            debug!(target: "kumandra-party", "Running session {:?} as authority id {:?}", session_id, node_id);
            Some(
            ...
            {
                warn!(target: "kumandra-party", "Failed to start nonvalidator session{:?}:{:?}", session_id, e);
            }
            None
        };
        ...
        loop {
            tokio::select! {
                _ = &mut check_session_status => {
                    let last_finalized_number = self.client.info().finalized_number;
                    if last_finalized_number >= last_block {
                        debug!(target: "kumandra-party", "Terminating session {:?}", session_id);
                        break;
                    }
                ...
                Some(next_session_authorities) = async {
                    match &mut start_next_session_network {
                        Some(notification) => {
                            match notification.await {
                                Err(e) => {
                                    warn!(target: "kumandra-party", "Error with subscription {:?}", e);
                    ...
                    match get_node_index(&next_session_authorities, self.keystore.clone()).await {
                        Some(node_id) => {
                            ...
                            {
                                warn!(target: "kumandra-party", "Failed to early start validator session{:?}:{:?}", next_session_id, e);
                            }
                        }
                        None => {
                            if let Err(e) = self
                            ...
                            {
                                warn!(target: "kumandra-party", "Failed to early start nonvalidator session{:?}:{:?}", next_session_id, e);
                            }
                        }
                    }
                ...
                Some(_) = async {
                    ...
                    warn!(target: "kumandra-party", "Authority task ended prematurely, giving up for this session.");
                    maybe_authority_task = None;
                },
            }
        }
        if let Some(task) = maybe_authority_task {
            debug!(target: "kumandra-party", "Stopping the authority task.");
            task.stop().await;
        }
        if let Err(e) = self.session_manager.stop_session(session_id) {
            warn!(target: "kumandra-party", "Session Manager failed to stop in session {:?}: {:?}", session_id, e)
        }
    }
    ...
    pub async fn run(mut self) {
        ...
        for curr_id in starting_session.0.. {
            info!(target: "kumandra-party", "Running session {:?}.", curr_id);
            self.run_session(SessionId(curr_id)).await;
        }
    }
}

pub(crate) fn create_kumandra_config(
    n_members: usize,
    node_id: NodeIndex,
    session_id: SessionId,
    unit_creation_delay: UnitCreationDelay,
) -> kumandra_bft::Config {
    let mut consensus_config = default_kumandra_config(n_members.into(), node_id, session_id.0 as u64);

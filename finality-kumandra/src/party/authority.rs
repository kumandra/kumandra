...
impl Subtasks {
    ...
    async fn stop(self) {
        ...
        debug!(target: "kumandra-party", "Started to stop all tasks");
        self.member.stop().await;
        trace!(target: "kumandra-party", "Member stopped");
        self.aggregator.stop().await;
        trace!(target: "kumandra-party", "Aggregator stopped");
        self.refresher.stop().await;
        trace!(target: "kumandra-party", "Refresher stopped");
        self.data_store.stop().await;
        trace!(target: "kumandra-party", "DataStore stopped");
    }
    ...
    pub async fn failed(mut self) -> bool {
        ...
        if result {
            debug!(target: "kumandra-party", "Something died and it was unexpected");
        }
        self.stop().await;
        debug!(target: "kumandra-party", "Stopped all processes");
        result
    }
}

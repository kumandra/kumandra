...
pub struct KumandraFinalizer<B, BE, C>
where
    B: Block,
    BE: Backend<B>,
...
impl<B, BE, C> KumandraFinalizer<B, BE, C>
where
    B: Block,
    BE: Backend<B>,
    C: HeaderBackend<B> + LockImportRun<B, BE> + Finalizer<B, BE>,
...
impl<B, BE, C> BlockFinalizer<B> for KumandraFinalizer<B, BE, C>
where
    B: Block,
    BE: Backend<B>,
    C: HeaderBackend<B> + LockImportRun<B, BE> + Finalizer<B, BE>,
{
    fn finalize_block(
        ...
        if status.finalized_number >= block_number {
            warn!(target: "kumandra-finality", "trying to finalize a block with hash {} and number {}
               that is not greater than already finalized {}", hash, block_number, status.finalized_number);
        }

        debug!(target: "kumandra-finality", "Finalizing block with hash {:?} and number {:?}. Previous best: #{:?}.", hash, block_number, status.finalized_number);
        ...
        let status = self.client.info();
        debug!(target: "kumandra-finality", "Attempted to finalize block with hash {:?}. Current best: #{:?}.", hash, status.finalized_number);
        update_res
    }
}

use crate::network::{
    ...
    KUMANDRA_PROTOCOL_NAME, KUMANDRA_VALIDATOR_PROTOCOL_NAME,
};
                ...
                if let Some(data) = receiver.next().await {
                    let sender = if let Some(sender) = senders.get(&protocol.name()) {
                        ...
                        match network.sender(peer_id, protocol.name()) {
                            ...
                            Err(e) => {
                                debug!(target: "kumandra-network", "Failed creating sender. Dropping message: {:?}", e);
                                continue;
                            }
                        }
                    };
                    if let Err(e) = sender.send(data.encode()).await {
                        debug!(target: "kumandra-network", "Failed sending data to peer. Dropping sender and message: {:?}", e);
                        senders.remove(&protocol.name());
                    }
                } else {
                    debug!(target: "kumandra-network", "Sender was dropped for peer {:?}. Peer sender exiting.", peer_id);
                    return;
                }
            }
        }
                ...
                match sender.unbounded_send(data) {
                    Err(e) => {
                        ...
                        if e.is_disconnected() {
                            trace!(target: "kumandra-network", "Failed sending data to peer because peer_sender receiver is dropped: {:?}", peer);
                        }
    ...
    fn broadcast(&mut self, data: D) {
        for peer in self.generic_connected_peers.clone() {
            ...
            if let Err(e) = self.send_to_peer(data.clone(), peer, Protocol::Generic) {
                trace!(target: "kumandra-network", "Failed to send broadcast to peer{:?}, {:?}", peer, e);
            }
        }
    }
    ...
    fn handle_network_event(&mut self, event: Event) -> Result<(), mpsc::TrySendError<D>> {
        match event {
            Event::SyncConnected { remote } => {
                trace!(target: "kumandra-network", "SyncConnected event for peer {:?}", remote);
                let addr = iter::once(multiaddr::Protocol::P2p(remote.into())).collect();
                self.network.add_reserved(
                    iter::once(addr).collect(),
                    Cow::Borrowed(KUMANDRA_PROTOCOL_NAME),
                );
            }
            Event::SyncDisconnected { remote } => {
                trace!(target: "kumandra-network", "SyncDisconnected event for peer {:?}", remote);
            ...
            Event::NotificationStreamOpened {
                ...
                Ok(Protocol::Generic) => {
                    trace!(target: "kumandra-network", "NotificationStreamOpened event for peer {:?} and protocol {:?}", remote, protocol);
                    let (tx, rx) = tracing_unbounded("mpsc_notification_stream_generic");
                    self.spawn_handle.spawn(
                        "kumandra/network/peer_sender",
                        None,
            ...
            Event::NotificationStreamClosed { remote, protocol } => {
                match protocol.as_ref().try_into() {
                    Ok(Protocol::Generic) => {
                        trace!(target: "kumandra-network", "NotificationStreamClosed event for peer {:?} and protocol {:?}", remote, protocol);
            ...
            Event::NotificationsReceived {
                ...
                for (protocol, data) in messages.into_iter() {
                    if protocol == KUMANDRA_PROTOCOL_NAME || protocol == KUMANDRA_VALIDATOR_PROTOCOL_NAME
                    {
                        match D::decode(&mut &data[..]) {
                            Ok(message) => self.messages_for_user.unbounded_send(message)?,
                            Err(e) => {
                                warn!(target: "kumandra-network", "Error decoding message: {}", e)
                            }
                        }
                    }
                }
    ...
    fn on_manager_command(&self, command: ConnectionCommand) {
        ...
        match command {
            AddReserved(addresses) => {
                self.network
                    .add_reserved(addresses, Cow::Borrowed(KUMANDRA_VALIDATOR_PROTOCOL_NAME));
            }
            DelReserved(peers) => self
                .network
                .remove_reserved(peers, Cow::Borrowed(KUMANDRA_VALIDATOR_PROTOCOL_NAME)),
        }
    }
        ...
        match command {
            ...
            SendTo(peer, protocol) => {
                if let Err(e) = self.send_to_peer(data, peer, protocol.clone()) {
                    trace!(target: "kumandra-network", "Failed to send data to peer{:?} via protocol {:?}, {:?}", peer, protocol, e);
                }
            }
        }
    }
            ...
            tokio::select! {
                maybe_event = events_from_network.next() => match maybe_event {
                    Some(event) => if let Err(e) = self.handle_network_event(event) {
                        error!(target: "kumandra-network", "Cannot forward messages to user: {:?}", e);
                        return;
                    },
                    None => {
                        error!(target: "kumandra-network", "Network event stream ended.");
                        return;
                    }
                },
                maybe_command = self.commands_from_manager.next() => match maybe_command {
                    ...
                    None => {
                        error!(target: "kumandra-network", "Manager command stream ended.");
                        return;
                    }
                },
                maybe_message = self.messages_from_user.next() => match maybe_message {
                    ...
                    None => {
                        error!(target: "kumandra-network", "User message stream ended.");
                        return;
                    }
                },
            }
...
mod tests {
    ...
    use crate::network::{
        ...
        NetworkIdentity, Protocol, KUMANDRA_PROTOCOL_NAME, KUMANDRA_VALIDATOR_PROTOCOL_NAME,
    };
    use codec::Encode;
        ...
        identities.iter().for_each(|identity| {
            test_data
                ...
                .emit_event(Event::NotificationStreamOpened {
                    protocol: Cow::Borrowed(KUMANDRA_PROTOCOL_NAME),
                    remote: identity.1.into(),
        ...
        let expected_messages = HashSet::from_iter(
            identities
                ...
                .map(|identity| {
                    messages
                        .iter()
                        .map(move |m| (m.clone(), identity.1, Cow::Borrowed(KUMANDRA_PROTOCOL_NAME)))
                })
                .flatten(),
        );
    ...
    async fn test_validator_data_command_send_to() {
        ...
        test_data
            .network
            .emit_event(Event::NotificationStreamOpened {
                protocol: Cow::Borrowed(KUMANDRA_VALIDATOR_PROTOCOL_NAME),
                remote: identity.1.into(),
                negotiated_fallback: None,
                role: ObservedRole::Authority,
            });
    ...
    async fn test_command_add_reserved() {
        ...
        let expected = (
            ...
            Cow::Borrowed(KUMANDRA_VALIDATOR_PROTOCOL_NAME),
        );

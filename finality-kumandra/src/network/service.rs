    ...
    fn handle_network_event(
        ...
        match event {
            Connected(multiaddress) => {
                trace!(target: "kumandra-network", "Connected event from address {:?}", multiaddress);
                self.network
                    .add_reserved(iter::once(multiaddress).collect(), Protocol::Generic);
            }
            Disconnected(peer) => {
                trace!(target: "kumandra-network", "Disconnected event for peer {:?}", peer);
                self.network
                    .remove_reserved(iter::once(peer).collect(), Protocol::Generic);
            }
            StreamOpened(peer, protocol) => {
                trace!(target: "kumandra-network", "StreamOpened event for peer {:?} and the protocol {:?}.", peer, protocol);
            ...
            StreamClosed(peer, protocol) => {
                trace!(target: "kumandra-network", "StreamClosed event for peer {:?} and protocol {:?}", peer, protocol);
                match protocol {
                    Protocol::Generic => {
            ...
            Messages(messages) => {
                for data in messages.into_iter() {
                    match D::decode(&mut &data[..]) {
                        ...
                        Err(e) => warn!(target: "kumandra-network", "Error decoding message: {}", e),
                    }
                }
            }
        }

    ...
    match receiver.lock().await.next().await {
        Some(Split::Left(data)) => {
            if left_sender.unbounded_send(data).is_err() {
                warn!(target: "kumandra-network", "Failed send despite controlling receiver, this shouldn't've happened.");
            }
            true
        }
        ...
        None => {
            trace!(target: "kumandra-network", "Split data channel ended");
...
///
/// The main example for now is creating an `kumandra_bft::Network` and a separate one for accumulating

...
impl<NI: NetworkIdentity, D: Data> Service<NI, D> {
    ...
    fn addresses(&self) -> Vec<Multiaddr> {
        ...
        debug!(target: "kumandra-network", "Got addresses:\n{:?}\n and peer_id:{:?}", addresses, peer_id);
        addresses
            .into_iter()
            .map(Multiaddr)
            ...
            Ok((maybe_command, data, data_from_network)) => {
                if let Some(result_for_user) = result_for_user {
                    if result_for_user.send(data_from_network).is_err() {
                        warn!(target: "kumandra-network", "Failed to send started session.")
                    }
                }
        ...
        match self.sessions.get_mut(&session_id) {
            Some(Session {
                ...
                let maybe_command = match !addresses.is_empty() && handler.is_validator() {
                    true => {
                        debug!(target: "kumandra-network", "Adding addresses for session {:?} to reserved: {:?}", session_id, addresses);
                        self.connections
            ...
            None => {
                debug!(target: "kumandra-network", "Received message from unknown session: {:?}", message);
                (None, Vec::new())
            }
        }
    }
...
impl<D: Data> IO<D> {
    ...
    pub async fn run<NI: NetworkIdentity>(
        ...
        loop {
            trace!(target: "kumandra-network", "Manager Loop started a next iteration");
            tokio::select! {
                maybe_command = self.commands_from_user.next() => {
                    trace!(target: "kumandra-network", "Manager received a command from user");
                    match maybe_command {
                        Some(command) => match service.on_command(command).await {
                            Ok(to_send) => self.send(to_send)?,
                            Err(e) => warn!(target: "kumandra-network", "Failed to update handler: {:?}", e),
                        },
                        None => return Err(Error::CommandsChannel),
                    }
                },
                maybe_message = self.messages_from_user.next() => {
                    trace!(target: "kumandra-network", "Manager received a message from user");
                    match maybe_message {
                ...
                maybe_message = self.messages_from_network.next() => {
                    trace!(target: "kumandra-network", "Manager received a message from network");
                    match maybe_message {
                        Some(message) => if let Err(e) = self.on_network_message(&mut service, message) {
                            match e {
                                Error::UserSend => trace!(target: "kumandra-network", "Failed to send to user in session."),
                                Error::NoSession => trace!(target: "kumandra-network", "Received message for unknown session."),
                                _ => return Err(e),
                            }
                        },
                ...
                _ = maintenance.tick() => {
                    debug!(target: "kumandra-network", "Manager starts maintenence");
                    match service.retry_session_start().await {
                        Ok(to_send) => self.send(to_send)?,
                        Err(e) => warn!(target: "kumandra-network", "Retry failed to update handler: {:?}", e),
                    }

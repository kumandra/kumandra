...
impl Discovery {
    ...
    pub fn discover_authorities(&mut self, handler: &SessionHandler) -> Vec<DiscoveryCommand> {
        ...
        debug!(target: "kumandra-network", "{:?}/{:?} authorities known for session {:?}.", node_count.0-missing_authorities.len(), node_count, handler.session_id());
    ...
    fn handle_broadcast(
        ...
        debug!(target: "kumandra-network", "Handling broadcast with authentication {:?}.", authentication);
        ...
        match handler.peer_id(&node_id) {
            ...
            None => {
                warn!(target: "kumandra-network", "Id of correctly authenticated peer not present.")
            }
        }
        if self.should_rebroadcast(&node_id) {
            trace!(target: "kumandra-network", "Rebroadcasting {:?}.", authentication);

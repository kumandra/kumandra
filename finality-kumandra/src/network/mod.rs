...
pub use kumandra::{NetworkData as KumandraNetworkData, NetworkWrapper};
pub use component::{
...
/// Name of the network protocol used by Kumandra. This is how messages
/// are subscribed to ensure that we are gossiping and communicating with our
/// own network.
const KUMANDRA_PROTOCOL_NAME: &str = "/cardinals/kumandra/2";
...
const KUMANDRA_VALIDATOR_PROTOCOL_NAME: &str = "/cardinals/KUMANDRA_Validator/1";
...
impl Protocol {
    pub fn name(&self) -> Cow<'static, str> {
        use Protocol::*;
        match self {
            Generic => Cow::Borrowed(KUMANDRA_PROTOCOL_NAME),
            Validator => Cow::Borrowed(KUMANDRA_VALIDATOR_PROTOCOL_NAME),
        }
    }
}
...
impl TryFrom<&str> for Protocol {
    ...
    fn try_from(item: &str) -> Result<Self, Self::Error> {
        match item {
            KUMANDRA_PROTOCOL_NAME => Ok(Protocol::Generic),
            KUMANDRA_VALIDATOR_PROTOCOL_NAME => Ok(Protocol::Validator),

...
pub enum KumandraData<B: BlockT> {
    Empty,
    HeadProposal(UnvalidatedKumandraProposal<B>),
}

// Need to be implemented manually, as deriving does not work (`BlockT` is not `Hash`).
impl<B: BlockT> Hash for KumandraData<B> {
    fn hash<H: Hasher>(&self, state: &mut H) {
        match self {
            KumandraData::Empty => {
                (0u8).hash(state);
            }
            KumandraData::HeadProposal(proposal) => {
                (1u8).hash(state);
                proposal.hash(state);
            }
        }
...
impl<B: BlockT> PartialEq for KumandraData<B> {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (KumandraData::Empty, KumandraData::Empty) => true,
            (KumandraData::HeadProposal(p1), KumandraData::HeadProposal(p2)) => p1.eq(p2),
            _ => false,
        }
    }
}

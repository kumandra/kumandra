use crate::{
    data_io::{
        ...
        proposal::{KumandraProposal, ProposalStatus},
        status_provider::get_proposal_status,
        KumandraNetworkMessage,
    },
...
#[derive(PartialEq, Eq, Clone, Debug)]
pub struct PendingMessageInfo<B: BlockT, M: KumandraNetworkMessage<B>> {
    message: M,
    // Data items that we still wait for
    pending_proposals: HashSet<KumandraProposal<B>>,
}

impl<B: BlockT, M: KumandraNetworkMessage<B>> PendingMessageInfo<B, M> {
...
where
    ...
    Message:
        KumandraNetworkMessage<B> + std::fmt::Debug + Send + Sync + Clone + codec::Codec + 'static,
    R: ReceiverComponent<Message>,
{
    next_free_id: MessageId,
    pending_proposals: HashMap<KumandraProposal<B>, PendingProposalInfo<B>>,
    event_triggers: HashMap<ChainEvent<B>, HashSet<KumandraProposal<B>>>,
    ...
    available_proposals_cache: LruCache<KumandraProposal<B>, ProposalStatus<B>>,
    ...
    messages_for_kumandra: UnboundedSender<Message>,
}
    ...
    R: ReceiverComponent<Message>,
{
    ...
    pub fn new<N: ComponentNetwork<Message, R = R>>(
        ...
        let (messages_for_kumandra, messages_from_data_store) = mpsc::unbounded();
        ...
        loop {
            ...
            tokio::select! {
                Some(message) = async {
                    ...
                    trace!(target: "kumandra-data-store", "Received message at Data Store {:?}", message);
                    self.on_message_received(message);
                }
                Some(block) = &mut import_stream.next() => {
                    trace!(target: "kumandra-data-store", "Block import notification at Data Store for block {:?}", block);
                    self.on_block_imported((block.header.hash(), *block.header.number()).into());
                },
                Some(block) = &mut finality_stream.next() => {
                    trace!(target: "kumandra-data-store", "Finalized block import notification at Data Store for block {:?}", block);
                ...
                _ = &mut exit => {
                    debug!(target: "kumandra-data-store", "Data Store task received exit signal. Terminating.");
                    break;
                }
            }
        }
    ...
    fn run_maintenance(&mut self) {
        ...
        match proposals_with_timestamps.len() {
            0 => {
                trace!(target: "kumandra-data-store", "No pending proposals in data store during maintenance.");
            }
            1..=5 => {
                info!(target: "kumandra-data-store", "Data Store maintenance. Awaiting {:?} proposals: {:?}",proposals_with_timestamps.len(), proposals_with_timestamps);
            }
            _ => {
                info!(target: "kumandra-data-store", "Data Store maintenance. Awaiting {:?} proposals: (showing 5 initial only) {:?}",proposals_with_timestamps.len(), &proposals_with_timestamps[..5]);
            }
        }
        ...
        for (proposal, first_occurrence) in proposals_with_timestamps {
            ...
            if !self.chain_info_provider.is_block_imported(&block) {
                debug!(target: "kumandra-data-store", "Requesting a stale block {:?} after it has been missing for {:?} secs.", block, time_waiting.as_secs());
            ...
            let parent_hash = match self.chain_info_provider.get_parent_hash(&bottom_block) {
                Ok(ph) => ph,
                _ => {
                    warn!(target: "kumandra-data-store", "Expected the block below the proposal {:?} to be imported", proposal);
                    continue;
                }
            };
            ...
            if let Ok(finalized_block) = self.chain_info_provider.get_finalized_at(parent_num) {
                if parent_hash != finalized_block.hash {
                    warn!(target: "kumandra-data-store", "The proposal {:?} is pending because the parent: \
                        {:?}, does not agree with the block finalized at this height: {:?}.", proposal, parent_hash, finalized_block);
                } else {
                    warn!(target: "kumandra-data-store", "The proposal {:?} is pending even though blocks \
                            have been imported and parent was finalized.", proposal);
                }
            } else {
                debug!(target: "kumandra-data-store", "Requesting a justification for block {:?} {:?} \
    ...
    fn register_finality_trigger(&mut self, proposal: &KumandraProposal<B>, number: NumberFor<B>) {
        self.num_triggers_registered_since_last_pruning += 1;
    ...
    fn register_next_finality_trigger(&mut self, proposal: &KumandraProposal<B>) {
    ...
    fn on_proposal_available(&mut self, proposal: &KumandraProposal<B>) {
    ...
    fn bump_proposal(&mut self, proposal: &KumandraProposal<B>) -> bool {
    ...
    fn on_message_dependencies_resolved(&self, message: Message) {
        trace!(target: "kumandra-data-store", "Sending message from DataStore {:?}", message);
        if let Err(e) = self.messages_for_kumandra.unbounded_send(message) {
            error!(target: "kumandra-data-store", "Unable to send a ready message from DataStore {}", e);
        }
    }
    ...
    fn remove_proposal_from_pending_message(&mut self, proposal: &KumandraProposal<B>, id: MessageId) {
        let mut message_info = match self.pending_messages.remove(&id) {
            Some(message_info) => message_info,
            None => {
                warn!(target: "kumandra-data-store", "Message {:?} not found when resolving a proposal dependency {:?}.", id, proposal);
                return;
            }
        };
    ...
    fn remove_message_id_from_pending_proposal(
        ...
        if let Occupied(mut proposal_entry) = self.pending_proposals.entry(proposal.clone()) {
            ...
            warn!(target: "kumandra-data-store", "Proposal {:?} with id {:?} referenced in message does not exist", proposal, id);
        }
    }
    ...
    fn prune_single_message(&mut self) -> bool {
        ...
        if let Some(id) = maybe_id {
            ...
            warn!(target: "kumandra-data-store", "Tried to prune a message but there are none pending.");
            false
        }
    }
    ...
    fn prune_pending_messages(&mut self) {
            ...
            || self.pending_proposals.len() > self.config.max_proposals_pending
        {
            if !self.prune_single_message() {
                warn!(target: "kumandra-data-store", "Message pruning in DataStore failed. Moving on.");
                break;
            }
        }
    }
    ...
    fn on_message_received(&mut self, message: Message) {
        ...
        for data in message.included_data() {
            use crate::data_io::KumandraData::*;
            match data {
                Empty => {}

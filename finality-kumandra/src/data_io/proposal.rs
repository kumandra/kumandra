...
/// the branch length before we transform it into a safer `KumandraProposal` type that guarantees we will not
/// fail on  any integer over- or underflows.
/// We expect that honest nodes create UnvalidatedKumandraProposal {branch: [h_0, h_1, ..., h_n], number: num} objects
...
///     3) height(b_n) = num,
///     4) The parent of b_0 has been finalized (prior to creating this KumandraData).
/// Such an UnvalidatedKumandraProposal  object should be thought of as a proposal for block b_n to be finalized.
/// We refer for to `DataProvider` for a precise description of honest nodes' algorithm of creating proposals.
#[derive(Clone, Debug, Encode, Decode)]
pub struct UnvalidatedKumandraProposal<B: BlockT> {
    pub branch: Vec<B::Hash>,
    pub number: NumberFor<B>,
}

/// Represents possible invalid states as described in [UnvalidatedKumandraProposal].
...
impl<B: BlockT> Hash for UnvalidatedKumandraProposal<B> {
...
impl<B: BlockT> PartialEq for UnvalidatedKumandraProposal<B> {
    fn eq(&self, other: &Self) -> bool {
        self.number.eq(&other.number) && self.branch.eq(&other.branch)
    }
}

impl<B: BlockT> Eq for UnvalidatedKumandraProposal<B> {}

impl<B: BlockT> UnvalidatedKumandraProposal<B> {
    pub(crate) fn new(branch: Vec<B::Hash>, block_number: NumberFor<B>) -> Self {
        UnvalidatedKumandraProposal {
            branch,
            number: block_number,
        }
    }
    ...
    ) -> Result<KumandraProposal<B>, ValidationError<B>> {
        use ValidationError::*;
...
/// A version of UnvalidatedKumandraProposal that has been initially validated and fits
/// within session bounds.
#[derive(Clone, Debug, Encode, Decode)]
pub struct KumandraProposal<B: BlockT> {
    branch: Vec<B::Hash>,
    number: NumberFor<B>,
}

// Need to be implemented manually, as deriving does not work (`BlockT` is not `Hash`).
impl<B: BlockT> Hash for KumandraProposal<B> {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.branch.hash(state);
        self.number.hash(state);
    }
...
// Clippy does not allow deriving PartialEq when implementing Hash manually
impl<B: BlockT> PartialEq for KumandraProposal<B> {
    fn eq(&self, other: &Self) -> bool {
        self.number.eq(&other.number) && self.branch.eq(&other.branch)
    }
}

impl<B: BlockT> Eq for KumandraProposal<B> {}

impl<B: BlockT> Index<usize> for KumandraProposal<B> {
    type Output = B::Hash;
...
impl<B: BlockT> KumandraProposal<B> {
...
#[cfg(test)]
mod tests {
    use super::{UnvalidatedKumandraProposal, ValidationError::*};
    ...
    fn proposal_with_empty_branch_is_invalid() {
        ...
        let branch = vec![];
        let proposal = UnvalidatedKumandraProposal::new(branch, session_boundaries.first_block());
        assert_eq!(
    ...
    fn too_long_proposal_is_invalid() {
        ...
        let proposal = UnvalidatedKumandraProposal::new(branch, session_end);
        assert_eq!(
            proposal.validate_bounds(&session_boundaries),
    ...
    fn proposal_not_within_session_is_invalid() {
        ...
        let proposal = UnvalidatedKumandraProposal::new(branch.clone(), session_start);
        assert_eq!(
            proposal.validate_bounds(&session_boundaries),
        ...
        let proposal = UnvalidatedKumandraProposal::new(branch, session_end + 1);
        assert_eq!(
            proposal.validate_bounds(&session_boundaries),
            Err(BlockOutsideSessionBoundaries {
                session_start,
    ...
    fn proposal_starting_at_zero_block_is_invalid() {
        ...
        let proposal = UnvalidatedKumandraProposal::new(branch, 1);
        assert_eq!(
            proposal.validate_bounds(&session_boundaries),
    ...
    fn valid_proposal_is_validated_positively() {
        ...
        let proposal = UnvalidatedKumandraProposal::new(branch, (MAX_DATA_BRANCH_LEN + 1) as u64);

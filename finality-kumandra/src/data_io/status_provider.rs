    ...
    CIP: ChainInfoProvider<B>,
{
    ...
    if is_hopeless_fork(chain_info_provider, proposal) {
        debug!(target: "kumandra-finality", "Encountered a hopeless fork proposal {:?}.", proposal);
        return Ignore;
    }
...
fn is_hopeless_fork<B, CIP>(chain_info_provider: &mut CIP, proposal: &KumandraProposal<B>) -> bool
where
    B: BlockT,
    CIP: ChainInfoProvider<B>,
{
...
fn is_ancestor_finalized<B, CIP>(chain_info_provider: &mut CIP, proposal: &KumandraProposal<B>) -> bool
where
    B: BlockT,
    CIP: ChainInfoProvider<B>,
{
...
mod tests {
    ...
    // A large number only for the purpose of creating `KumandraProposal`s
    const DUMMY_SESSION_LEN: u32 = 1_000_000;

    fn proposal_from_headers(headers: Vec<Header>) -> KumandraProposal<Block> {
    ...
    fn proposal_from_blocks(blocks: Vec<Block>) -> KumandraProposal<Block> {
    ...
    fn verify_proposal_status(
        ...
        proposal: &KumandraProposal<Block>,

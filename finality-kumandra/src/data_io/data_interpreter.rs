use crate::{
    data_io::{
        ...
        KumandraData, ChainInfoProvider,
    },
...
pub struct OrderedDataInterpreter<B: BlockT, C: HeaderBackend<B>> {
    ...
    last_finalized_by_kumandra: BlockHashNum<B>,
...
impl<B: BlockT, C: HeaderBackend<B>> OrderedDataInterpreter<B, C> {
    pub fn new(
        ...
        let last_finalized_by_kumandra =
            get_last_block_prev_session(session_boundaries.clone(), client.clone());
        let chain_info_provider =
            AuxFinalizationChainInfoProvider::new(client, last_finalized_by_kumandra.clone());
        let chain_info_provider =
        ...
        OrderedDataInterpreter {
            ...
            chain_info_provider,
            last_finalized_by_kumandra,
            session_boundaries,
        }
    }

    fn block_to_finalize_from_data(&mut self, new_data: KumandraData<B>) -> Option<BlockHashNum<B>> {
        match new_data {
            KumandraData::Empty => None,
            KumandraData::HeadProposal(unvalidated_proposal) => {
                let proposal = match unvalidated_proposal.validate_bounds(&self.session_boundaries)
                {
                    Ok(proposal) => proposal,
                    Err(error) => {
                        warn!(target: "kumandra-finality", "Incorrect proposal {:?} passed through data availability, session bounds: {:?}, error: {:?}", unvalidated_proposal, self.session_boundaries, error);
                        return None;
                    }
                };
                ...
                match status {
                    ...
                    Ignore => {
                        debug!(target: "kumandra-finality", "Ignoring proposal {:?} in interpreter.", proposal);
                        None
                    }
...
#[async_trait]
impl<B: BlockT, C: HeaderBackend<B> + Send + 'static> kumandra_bft::FinalizationHandler<KumandraData<B>>
    for OrderedDataInterpreter<B, C>
{
    async fn data_finalized(&mut self, data: KumandraData<B>) {
        if let Some(block) = self.block_to_finalize_from_data(data) {
            self.last_finalized_by_kumandra = block.clone();
            self.chain_info_provider
                .inner()
                .update_aux_finalized(block.clone());
            if let Err(err) = self.blocks_to_finalize_tx.unbounded_send(block) {
                error!(target: "kumandra-finality", "Error in sending a block from FinalizationHandler, {}", err);
            }
        }
    }
}

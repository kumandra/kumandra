use crate::{
    data_io::{proposal::UnvalidatedKumandraProposal, KumandraData, MAX_DATA_BRANCH_LEN},
    metrics::Checkpoint,
    ...
    C: HeaderBackend<B>,
{
        ...
        .expect("client must respond")
    {
        ...
        warn!(target: "kumandra-data-store", "Trying to fetch the parent of an unknown block {:?}.", block);
        None
    }
}
    ...
    C: HeaderBackend<B>,
{
    ...
    if curr_block.hash == finalized_block.hash {
        ...
        branch.reverse();
        Ok(KumandraData::HeadProposal(UnvalidatedKumandraProposal::<B>::new(
            branch, num_last,
        )))
    } else {
        ...
        warn!(target: "kumandra-data-store", "Error computing proposal. Conflicting blocks: {:?}, finalized {:?}", curr_block, finalized_block);
        Err(())
    }
}
...
/// ChainTracker keeps track of the best_block in a given session and allows to generate `KumandraData`.
    ...
    SC: SelectChain<B> + 'static,
{
    ...
    ) -> (Self, impl kumandra_bft::DataProvider<KumandraData<B>>) {
        let data_to_propose = Arc::new(Mutex::new(KumandraData::Empty));
        (
            ChainTracker {
                select_chain,
                client,
    ...
    async fn update_data(&mut self, best_block_in_session: &BlockHashNum<B>) {
        ...
        if finalized_block.num >= self.session_boundaries.last_block() {
            ...
            *self.data_to_propose.lock().await = KumandraData::Empty;
            return;
        }
        ...
        if best_block_in_session.num < finalized_block.num {
            ...
            warn!(target: "kumandra-data-store", "Error updating data. best_block {:?} is lower than finalized {:?}.", best_block_in_session, finalized_block);
            return;
        }
    ...
    pub async fn run(mut self, mut exit: oneshot::Receiver<()>) {
        ...
        loop {
            ...
            tokio::select! {
                ...
                _ = &mut exit => {
                    debug!(target: "kumandra-data-store", "Task for refreshing best chain received exit signal. Terminating.");
                    return;
                }
            }
        }
...
#[async_trait]
impl<B: BlockT> kumandra_bft::DataProvider<KumandraData<B>> for DataProvider<B> {
    async fn get_data(&mut self) -> KumandraData<B> {
        let data = (*self.data_to_propose.lock().await).clone();

        if let Some(m) = &self.metrics {
            if let KumandraData::HeadProposal(proposal) = &data {
                m.report_block(
        ...
        debug!(target: "kumandra-data-store", "Outputting {:?} in get_data", data);
        data
    }
}
...
mod tests {
    use crate::{
        data_io::{
            ...
            KumandraData, MAX_DATA_BRANCH_LEN,
        },
        testing::{client_chain_builder::ClientChainBuilder, mocks::kumandra_data_from_blocks},
    ...
    fn prepare_chain_tracker_test() -> (
        ...
        oneshot::Sender<()>,
        ClientChainBuilder,
        impl kumandra_bft::DataProvider<KumandraData<Block>>,
    ) {
    ...
    where
        F: Future,
        S: FnOnce(ClientChainBuilder, Box<dyn kumandra_bft::DataProvider<KumandraData<Block>>>) -> F,
    {
    ...
    async fn proposes_empty_and_nonempty_when_expected() {
        run_test(|mut chain_builder, mut data_provider| async move {
            ...
            let expected_data = kumandra_data_from_blocks(blocks[..MAX_DATA_BRANCH_LEN].to_vec());
    ...
    async fn proposal_changes_with_finalization() {
        run_test(|mut chain_builder, mut data_provider| async move {
            ...
            for height in 1..(2 * MAX_DATA_BRANCH_LEN) {
                ...
                let data = data_provider.get_data().await;
                let expected_data =
                    kumandra_data_from_blocks(blocks[height..(MAX_DATA_BRANCH_LEN + height)].to_vec());
                assert_eq!(data, expected_data);
            }
    ...
    async fn returns_empty_proposal_above_session_end() {
        run_test(|mut chain_builder, mut data_provider| async move {
            ...
            sleep_enough().await;
            let data = data_provider.get_data().await;
            let expected_data = kumandra_data_from_blocks(blocks[0..MAX_DATA_BRANCH_LEN].to_vec());
            assert_eq!(data, expected_data);

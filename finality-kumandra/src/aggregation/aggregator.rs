...
impl<H: Copy + Hash, PMS> BlockSignatureAggregator<H, PMS> {
    ...
    fn on_multisigned_hash(&mut self, hash: H, signature: PMS) {
        debug!(target: "kumandra-aggregator", "New multisigned_hash {:?}.", hash);
    ...
    > IO<H, D, N, PMS, RMC>
{
    ...
    pub(crate) async fn start_aggregation(&mut self, hash: H) {
        debug!(target: "kumandra-aggregator", "Started aggregation for block hash {:?}", hash);
        if let Err(AggregatorError::DuplicateHash) = self.aggregator.on_start(hash) {
            debug!(target: "kumandra-aggregator", "Aggregation already started for block hash {:?}, ignoring.", hash);
            return;
        }
        self.multicast
        ...
        loop {
            tokio::select! {
                ...
                message_from_rmc = self.messages_from_rmc.next() => {
                    trace!(target: "kumandra-aggregator", "Our rmc message {:?}.", message_from_rmc);
                    match message_from_rmc {
                        ...
                        None => {
                            warn!(target: "kumandra-aggregator", "the channel of messages from rmc closed");
                        }
                    }
                }
                message_from_network = self.network.next() =>
                    match message_from_network {
                        Some(message_from_network) => {
                            trace!(target: "kumandra-aggregator", "Received message for rmc: {:?}", message_from_network);
    ...
    pub(crate) async fn next_multisigned_hash(&mut self) -> Option<(H, PMS)> {
        loop {
            trace!(target: "kumandra-aggregator", "Entering next_multisigned_hash loop.");
            ...
            if self.wait_for_next_signature().await.is_err() {
                warn!(target: "kumandra-aggregator", "the network channel closed");
                return None;
            }
        }
    }

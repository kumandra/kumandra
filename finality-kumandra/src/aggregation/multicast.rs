...
//! and modular way.
//!
//! We expose the `Multicast` trait, mimicking the interface of `kumandra_bft::ReliableMulticast`

use crate::crypto::{KeyBox, Signature};
use kumandra_bft::{Signable, SignatureSet};
use kumandra_bft_rmc::ReliableMulticast;
...
/// Anything that exposes the same interface as `kumandra_bft::ReliableMulticast`.
///
/// The trait defines an associated type: `Signed`. For `ReliableMulticast`, this is simply
/// `kumandra_bft::Multisigned` but the mocks are free to use the simplest matching implementation.

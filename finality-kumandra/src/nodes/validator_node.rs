...
pub async fn run_validator_node<B, H, C, BE, SC>(kumandra_config: KumandraConfig<B, H, C, SC>)
where
    B: Block,
    H: ExHashT,
    ...
    SC: SelectChain<B> + 'static,
{
    ...
    spawn_handle.spawn("kumandra/justification_handler", None, handler_task);
    debug!(target: "kumandra-party", "JustificationHandler has started.");

    spawn_handle.spawn("kumandra/network_manager", None, network_manager_task);
    spawn_handle.spawn("kumandra/network", None, network_task);
    debug!(target: "kumandra-party", "Network has started.");
    ...
    debug!(target: "kumandra-party", "Consensus party has started.");
    party.run().await;
    error!(target: "kumandra-party", "Consensus party has finished unexpectedly.");
}

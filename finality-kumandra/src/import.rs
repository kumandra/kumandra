...
use kumandra_primitives::KUMANDRA_ENGINE_ID;
...
pub struct KumandraBlockImport<Block, Be, I>
where
    Block: BlockT,
    Be: Backend<Block>,
    I: crate::ClientForKumandra<Block, Be>,
{
    inner: Arc<I>,
...
impl<Block, Be, I> KumandraBlockImport<Block, Be, I>
where
    Block: BlockT,
    Be: Backend<Block>,
    I: crate::ClientForKumandra<Block, Be>,
{
    pub fn new(
        ...
        justification_tx: UnboundedSender<JustificationNotification<Block>>,
        metrics: Option<Metrics<<Block::Header as Header>::Hash>>,
    ) -> KumandraBlockImport<Block, Be, I> {
        KumandraBlockImport {
            inner,
            justification_tx,
            metrics,
    ...
    fn send_justification(
        ...
        debug!(target: "kumandra-justification", "Importing justification for block {:?}", number);
        if justification.0 != KUMANDRA_ENGINE_ID {
            return Err(SendJustificationError::Consensus(Box::new(
                ConsensusError::ClientImport("Kumandra can import only Kumandra justifications.".into()),
            )));
        }
        let justification_raw = justification.1;
        let kumandra_justification = match backwards_compatible_decode(justification_raw) {
            JustificationDecoding::V1(just) => {
                debug!(target: "kumandra-justification", "Justification for block {:?} decoded correctly as V1", number);
                just.into()
            }
        ...
        self.justification_tx
            .unbounded_send(JustificationNotification {
                hash,
                number,
                justification: kumandra_justification,
            })
            .map_err(SendJustificationError::Send)
    }
}

impl<Block, Be, I> Clone for KumandraBlockImport<Block, Be, I>
where
    Block: BlockT,
    Be: Backend<Block>,
    I: crate::ClientForKumandra<Block, Be>,
...
impl<Block, Be, I> BlockImport<Block> for KumandraBlockImport<Block, Be, I>
where
    Block: BlockT,
    Be: Backend<Block>,
    I: crate::ClientForKumandra<Block, Be> + Send,
    for<'a> &'a I:
    ...
    TransactionFor<I, Block>: Send + 'static,
{
    ...
    async fn import_block(
        ...
        debug!(target: "kumandra-justification", "Importing block {:?} {:?} {:?}", number, block.header.hash(), block.post_hash());
        ...
        if let Some(justification) =
            justifications.and_then(|just| just.into_justification(KUMANDRA_ENGINE_ID))
        {
            debug!(target: "kumandra-justification", "Got justification along imported block {:?}", number);

            if let Err(e) =
                self.send_justification(post_hash, number, (KUMANDRA_ENGINE_ID, justification))
            {
                warn!(target: "kumandra-justification", "Error while receiving justification for block {:?}: {:?}", post_hash, e);
            }
        }
...
#[async_trait::async_trait]
impl<Block, Be, I> JustificationImport<Block> for KumandraBlockImport<Block, Be, I>
where
    Block: BlockT,
    Be: Backend<Block>,
    I: crate::ClientForKumandra<Block, Be>,
{
    ...
    async fn on_start(&mut self) -> Vec<(Block::Hash, NumberFor<Block>)> {
        debug!(target: "kumandra-justification", "On start called");
        Vec::new()
    }
    ...
    async fn import_justification(
        ...
        debug!(target: "kumandra-justification", "import_justification called on {:?}", justification);
        self.send_justification(hash, number, justification)
            .map_err(|error| match error {
                ...
                SendJustificationError::Decode => {
                    warn!(target: "kumandra-justification", "Justification for block {:?} decoded incorrectly", number);

use crate::{
    data_io::{KumandraData, KumandraNetworkMessage, DataStore, DataStoreConfig, MAX_DATA_BRANCH_LEN},
    network::{DataNetwork, RequestBlocks, SimpleNetwork},
    session::{SessionBoundaries, SessionId, SessionPeriod},
    testing::{
        client_chain_builder::ClientChainBuilder,
        mocks::{kumandra_data_from_blocks, kumandra_data_from_headers},
    },
    BlockHashNum,
};
use futures::{
...
type TestData = Vec<KumandraData<Block>>;

impl KumandraNetworkMessage<Block> for TestData {
    fn included_data(&self) -> Vec<KumandraData<Block>> {
        self.clone()
    }
}
...
async fn correct_messages_go_through() {
    run_test(|mut test_handler| async move {
        ...
        for i in 1..=MAX_DATA_BRANCH_LEN {
            ...
            let test_data: TestData = vec![kumandra_data_from_blocks(blocks_branch)];
        ...
        for l in 0..MAX_DATA_BRANCH_LEN {
            for r in 0..MAX_DATA_BRANCH_LEN {
                    ...
                    && !(session_start <= left_end && right_end <= session_end)
                {
                    ...
                    test_handler.send_data(vec![kumandra_data_from_blocks(blocks_branch)]);
                }
            }
        }
    }
...
async fn message_with_multiple_data_gets_through_when_it_should() {
    run_test(|mut test_handler| async move {
        ...
        for i in 1..=max_height {
            ...
            test_data.push(kumandra_data_from_blocks(blocks_branch));
        }
        test_handler.send_data(test_data.clone());
...
async fn sends_justification_request_when_not_finalized() {
    run_test(|mut test_handler| async move {
        ...
        let test_data = vec![kumandra_data_from_blocks(blocks_branch)];
...
async fn message_with_genesis_block_does_not_get_through() {
    run_test(|mut test_handler| async move {
        ...
        for i in 1..MAX_DATA_BRANCH_LEN {
            let test_data: TestData = vec![kumandra_data_from_headers(
                (0..i)
                    .into_iter()
...
async fn hopeless_fork_at_the_boundary_goes_through() {
    run_test(|mut test_handler| async move {
        ...
        let honest_data = vec![kumandra_data_from_blocks(honest_hopeless_fork)];
        let honest_data2 = vec![kumandra_data_from_blocks(honest_hopeless_fork2)];
        let malicious_data = vec![kumandra_data_from_blocks(malicious_hopeless_fork)];
        let malicious_data2 = vec![kumandra_data_from_blocks(malicious_hopeless_fork2)];

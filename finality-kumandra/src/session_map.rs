use crate::{
    first_block_of_session, session_id_from_block_num, ClientForKumandra, SessionId, SessionPeriod,
};
use kumandra_primitives::{KumandraSessionApi, AuthorityId};
...
where
    C: ClientForKumandra<B, BE> + Send + Sync + 'static,
        ...
        if let Some(senders) = guard.1.remove(&id) {
            for sender in senders {
                if let Err(e) = sender.send(authorities.clone()) {
                    error!(target: "kumandra-session-updater", "Error while sending notification: {:?}", e);
                }
            }
        }
    ...
    B: Block,
{
    ...
    async fn handle_first_block_of_session(&mut self, num: NumberFor<B>, session_id: SessionId) {
        debug!(target: "kumandra-session-updater", "Handling first block #{:?} of session {:?}", num, session_id.0);
        ...
        if session_id.0 >= PRUNING_THRESHOLD && session_id.0 % PRUNING_THRESHOLD == 0 {
            debug!(target: "kumandra-session-updater", "Pruning session map below session #{:?}", session_id.0 - PRUNING_THRESHOLD);
            self.session_map
    ...
    pub async fn run(mut self, period: SessionPeriod) {
        ...
        while let Some(FinalityNotification { header, .. }) = notifications.next().await {
            ...
            trace!(target: "kumandra-session-updater", "got FinalityNotification about #{:?}", last_finalized);

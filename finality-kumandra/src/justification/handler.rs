    ...
    pub async fn run(
        ...
        loop {
            ...
            let SessionInfo {
                ...
                .await;
            if verifier.is_none() {
                debug!(target: "kumandra-justification", "Verifier for session {:?} not yet available. Waiting {}ms and will try again ...", current_session, self.verifier_timeout.as_millis());
...
fn wrap_channel_with_logging<B: BlockT>(
    ...
    channel
        .inspect(move |_| {
            debug!(target: "kumandra-justification", "Got justification ({})", label);
        })
        .chain(futures::stream::iter(std::iter::from_fn(move || {
            error!(target: "kumandra-justification", "Justification ({}) stream ended.", label);
            None
        })))
}

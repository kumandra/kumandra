use crate::{
    ...
    justification::KumandraJustification,
};
use kumandra_bft::{PartialMultisignature, SignatureSet};
...
struct KumandraJustificationV1 {
    pub signature: SignatureSet<SignatureV1>,
}

impl From<KumandraJustificationV1> for KumandraJustification {
    fn from(justification: KumandraJustificationV1) -> KumandraJustification {
...
enum VersionedKumandraJustification {
    // Most likely from the future.
    Other(Version, Vec<u8>),
    V1(KumandraJustificationV1),
    V2(KumandraJustification),
}
...
impl Encode for VersionedKumandraJustification {
    fn size_hint(&self) -> usize {
        use VersionedKumandraJustification::*;
...
impl Decode for VersionedKumandraJustification {
    fn decode<I: CodecInput>(input: &mut I) -> Result<Self, CodecError> {
        ...
        match version {
            1 => Ok(V1(KumandraJustificationV1::decode(input)?)),
            2 => Ok(V2(KumandraJustification::decode(input)?)),
            _ => {
...
) -> Result<KumandraJustification, Error> {
    use Error::*;
    let justification_cloned = justification_raw.clone();
    match VersionedKumandraJustification::decode_all(&mut justification_cloned.as_slice()) {
        Ok(justification) => {
            use VersionedKumandraJustification::*;
            match justification {
        ...
        Err(_) => {
            ...
            let justification_cloned = justification_raw.clone();
            match KumandraJustification::decode_all(&mut justification_cloned.as_slice()) {
                Ok(justification) => Ok(justification),
                Err(_) => match KumandraJustificationV1::decode_all(&mut justification_raw.as_slice())
                {
...
pub fn versioned_encode(justification: KumandraJustification) -> Vec<u8> {
    VersionedKumandraJustification::V2(justification).encode()
}

#[cfg(test)]
mod test {
    use super::{backwards_compatible_decode, KumandraJustificationV1, VersionedKumandraJustification};
    use crate::{
        crypto::{Signature, SignatureV1},
        justification::KumandraJustification,
    };
    use kumandra_bft::{NodeCount, PartialMultisignature, SignatureSet};
    use kumandra_primitives::{AuthorityPair, AuthoritySignature};
    ...
    fn correctly_decodes_v2() {
        ...
        let just_v2 = KumandraJustification {
            signature: signature_set,
        };
    ...
    fn correctly_decodes_other() {
        let other = VersionedKumandraJustification::Other(43, vec![21, 37]);
        let encoded = other.encode();
        let decoded = VersionedKumandraJustification::decode(&mut encoded.as_slice());

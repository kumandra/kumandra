...
pub struct KumandraJustificationV1 {
...
pub fn backwards_compatible_decode(justification_raw: Vec<u8>) -> JustificationDecoding {
    ...
    if let Ok(justification) = KumandraJustification::decode_all(&mut justification_cloned.as_slice())
    {
        JustificationDecoding::V2(justification)
    } else if let Ok(justification) =
        KumandraJustificationV1::decode_all(&mut justification_raw.as_slice())
    {

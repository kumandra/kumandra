...
use kc_consensus::{
    ...
    RewardSigningNotification, KumandraLink, KumandraParams,
};
...
use std::sync::Arc;
use kumandra_fraud_proof::VerifyFraudProof;
use kumandra_runtime_primitives::opaque::Block;
use kumandra_runtime_primitives::{AccountId, Balance, Hash, Index as Nonce};

/// Error type for Kumandra service.
...
pub type FraudProofVerifier<RuntimeApi, ExecutorDispatch> = kumandra_fraud_proof::ProofVerifier<
    Block,
...
where
    ...
    RuntimeApi::RuntimeApi: ApiExt<Block, StateBackend = StateBackendFor<FullBackend, Block>>
        ...
        + SessionKeys<Block>
        + KumandraApi<Block, FarmerPublicKey>
        + ObjectsApi<Block>
        ...
        CanAuthorWithNativeVersion::new(client.executor().clone()),
        {
            ...
            move |parent_hash, kumandra_link: KumandraLink<Block>| {
                ...
                async move {
                    ...
                    Ok((timestamp, kumandra_inherents))
                }
            }
        },
    )?;
    ...
    ExecutorDispatch: NativeExecutionDispatch + 'static,
{
    ...
    let reward_signing_notification_stream = kumandra_link.reward_signing_notification_stream();

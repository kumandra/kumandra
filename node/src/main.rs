...
use kumandra_node::{new_authority, new_full, new_partial, Cli, Subcommand};
use clap::Parser;
...
fn main() -> sc_cli::Result<()> {
    ...
    match &cli.subcommand {
        ...
        None => {
            ...
            let kumandra_cli_config = cli.kumandra;
            runner.run_node_until_exit(|config| async move {
                match config.role {
                    Role::Authority => {
                        new_authority(config, kumandra_cli_config).map_err(sc_cli::Error::Service)
                    }
                    Role::Full => {
                        new_full(config, kumandra_cli_config).map_err(sc_cli::Error::Service)
                    }

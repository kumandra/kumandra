...
use kumandra_primitives::AuthorityId as KumandraId;
use kumandra_runtime::AccountId;
use clap::Parser;
...
/// returns Kumandra key, if absent a new key is generated
fn kumandra_key(keystore: &impl SyncCryptoStore) -> KumandraId {
    SyncCryptoStore::ed25519_public_keys(&*keystore, kumandra_primitives::KEY_TYPE)
        .pop()
        .unwrap_or_else(|| {
            SyncCryptoStore::ed25519_generate_new(&*keystore, kumandra_primitives::KEY_TYPE, None)
                .expect("Could not create Kumandra key")
        })
        .into()
}
...
fn authority_keys(
    ...
    let kumandra_key = kumandra_key(keystore);

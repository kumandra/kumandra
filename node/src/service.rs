...
use crate::{kumandra_cli::KumandraCli, executor::KumandraExecutor};
use kumandra_primitives::KumandraSessionApi;
use kumandra_runtime::{self, opaque::Block, RuntimeApi, MAX_BLOCK_SIZE};
use finality_kumandra::{
    run_nonvalidator_node, run_validator_node, KumandraBlockImport, KumandraConfig,
...
type FullClient = sc_service::TFullClient<Block, RuntimeApi, KumandraExecutor>;
...
pub fn new_partial(
    ...
    sc_service::PartialComponents<
        ...
        (
            KumandraBlockImport<Block, FullBackend, FullClient>,
...
> {
    ...
    let executor = KumandraExecutor::new(
        config.wasm_method,
    ...
    let (client, backend, keystore_container, task_manager) =
        sc_service::new_full_parts::<Block, RuntimeApi, KumandraExecutor>(
            config,
    ...
    let kumandra_block_import =
        KumandraBlockImport::new(client.clone() as Arc<_>, justification_tx, metrics.clone());

    let slot_duration = sc_consensus_aura::slot_duration(&*client)?;

    let import_queue = sc_consensus_aura::import_queue::<AuraPair, _, _, _, _, _, _>(
        ImportQueueParams {
            block_import: kumandra_block_import.clone(),
            justification_import: Some(Box::new(kumandra_block_import.clone())),
            client: client.clone(),
    ...
    Ok(sc_service::PartialComponents {
        ...
        select_chain,
        transaction_pool,
        other: (kumandra_block_import, justification_rx, telemetry, metrics),
    })
}
...
> {
    config
        .network
        .extra_sets
        .push(finality_kumandra::peers_set_config(Protocol::Generic));
...
pub fn new_authority(
    ...
    config
        .network
        .extra_sets
        .push(finality_kumandra::peers_set_config(Protocol::Validator));
    ...
    let unit_creation_delay = kumandra_config.unit_creation_delay();
    ...
    let kumandra_config = KumandraConfig {
        network,
        client,
        select_chain,
        session_period,
    ...
    task_manager.spawn_essential_handle().spawn_blocking(
        "kumandra",
        None,
        run_validator_node(kumandra_config),
    );
...
pub fn new_full(
    ...
    task_manager.spawn_essential_handle().spawn_blocking(
        "kumandra",
        None,
        run_nonvalidator_node(kumandra_config),
    );

...
use kumandra_node_runtime::RuntimeApi;
...
use kmc_consensus_rrsc::{self, SlotProportion};
...
pub fn new_partial(
	...
	sc_service::PartialComponents<
		...
		(
			(
				kmc_consensus_rrsc::RRSCBlockImport<Block, FullClient, FullGrandpaBlockImport>,
				grandpa::LinkHalf<Block, FullClient, FullSelectChain>,
				kmc_consensus_rrsc::RRSCLink<Block>,
			),
			Option<Telemetry>,
		),
	>,
...
> {
	...
	let (block_import, rrsc_link) = kmc_consensus_rrsc::block_import(
		kmc_consensus_rrsc::Config::get(&*client)?,
		grandpa_block_import,
		client.clone(),
	)?;

	let slot_duration = rrsc_link.config().slot_duration();
	let import_queue = kmc_consensus_rrsc::import_queue(
		rrsc_link.clone(),
		block_import.clone(),
		...
		move |_, ()| async move {
			...
			let slot =
				kmp_consensus_rrsc::inherents::InherentDataProvider::from_timestamp_and_slot_duration(
					*timestamp,
					slot_duration,
				);
...
pub fn new_full_base(
	...
	if let sc_service::config::Role::Authority { .. } = &role {
		...
		let rrsc_config = kmc_consensus_rrsc::RRSCParams {
			keystore: keystore_container.sync_keystore(),
			client: client.clone(),
			select_chain,
			env: proposer,
		...
		let rrsc = kmc_consensus_rrsc::start_rrsc(rrsc_config)?;

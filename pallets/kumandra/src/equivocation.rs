...
//!
//! This module defines an offence type for Kumandra equivocations and some utility traits to wire
//! together:
...
//! (from the client that's import Kumandra blocks). And in a runtime context, so that the Kumandra
...
//! `ValidateUnsigned` for the Kumandra pallet is used in the runtime definition.
//!

use frame_support::traits::Get;
use frame_system::offchain::SubmitTransaction;
use sp_consensus_slots::Slot;
use kp_consensus::offence::{Kind, Offence, OffenceError, ReportOffence};
...
/// A trait with utility methods for handling equivocation reports in Kumandra. The trait provides
...
pub trait HandleEquivocation<T: Config> {
    ...
    fn report_offence(
        offence: KumandraEquivocationOffence<FarmerPublicKey>,
    ) -> Result<(), OffenceError>;
...
impl<T: Config> HandleEquivocation<T> for () {
    ...
    fn report_offence(
        _offence: KumandraEquivocationOffence<FarmerPublicKey>,
    ) -> Result<(), OffenceError> {
        Ok(())
    }
...
where
    ...
    // processed.
    R: ReportOffence<FarmerPublicKey, KumandraEquivocationOffence<FarmerPublicKey>>,
    // The longevity (in blocks) that the equivocation report is valid for. When using the staking
...
impl<T: Config> Pallet<T> {
    pub fn validate_equivocation_report(
        ...
        ValidTransaction::with_tag_prefix("KumandraEquivocation")
...
/// A Kumandra equivocation offence report.
///
/// When a farmer released two or more blocks at the same slot.
#[derive(Debug, Eq, PartialEq)]
pub struct KumandraEquivocationOffence<PublicKey> {
    /// A Kumandra slot in which this incident happened.
    pub slot: Slot,
    /// Identity of the farmer that produced the equivocation.
    pub offender: PublicKey,
}

impl<PublicKey: Clone> Offence<PublicKey> for KumandraEquivocationOffence<PublicKey> {

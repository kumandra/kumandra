...
/// A type signifying to Kumandra that it should perform a global randomness update with an internal
/// trigger.
...
mod pallet {
    ...
    use sp_consensus_slots::Slot;
    use kp_consensus::inherents::{InherentError, InherentType, INHERENT_IDENTIFIER};
    use kp_consensus::{EquivocationProof, FarmerPublicKey, FarmerSignature, SignedVote};
    ...
    impl<T: Config> Get<kp_consensus::SolutionRanges> for InitialSolutionRanges<T> {
        fn get() -> kp_consensus::SolutionRanges {
            kp_consensus::SolutionRanges {
    ...
    pub trait Config: pallet_timestamp::Config {
        ...
        /// The expected average block time at which Kumandra should be creating blocks. Since
        /// Kumandra is probabilistic it is not trivial to figure out what the expected average
    ...
    pub(super) type GlobalRandomnesses<T> =
        StorageValue<_, kp_consensus::GlobalRandomnesses, ValueQuery>;
...
impl<T: Config> Pallet<T> {
    /// Determine the Kumandra slot duration based on the Timestamp module configuration.
    ...
    fn do_initialize(block_number: T::BlockNumber) {
        let pre_digest = <frame_system::Pallet<T>>::digest()
            .logs
            .iter()
            .find_map(|s| s.as_kumandra_pre_digest::<T::AccountId>())
...

        {
            ...
            if ParentBlockVoters::<T>::get().contains_key(&key) {
                ...
                let offence = KumandraEquivocationOffence {
                    slot,
                    offender: public_key,
                };
        ...
        if let Some(next_randomness) = GlobalRandomnesses::<T>::get().next {
            GlobalRandomnesses::<T>::put(kp_consensus::GlobalRandomnesses {
        ...
        if let kp_consensus::SolutionRanges {
            next: Some(next),
            voting_next: Some(voting_next),
            ..
        } = SolutionRanges::<T>::get()
        {
            SolutionRanges::<T>::put(kp_consensus::SolutionRanges {
                current: next,
                next: None,
        ...
        // Update current salt if needed
        {
            ...
            if salts.switch_next_block {
                if let Some(next_salt) = salts.next {
                    Salts::<T>::put(kp_consensus::Salts {
                        current: next_salt,
                        next: None,
    ...
    fn do_report_equivocation(
        ...
        if !kp_consensus::is_equivocation_proof_valid::<_, T::AccountId>(
            equivocation_proof,
        ) {
            return Err(Error::<T>::InvalidEquivocationProof.into());
        }

        let offence = KumandraEquivocationOffence { slot, offender };
    ...
    T: Config + SendTransactionTypes<Call<T>>,
{
    ...
    pub fn submit_vote(signed_vote: SignedVote<T::BlockNumber, T::Hash, T::AccountId>) {
        ...
        match SubmitTransaction::<T, Call<T>>::submit_unsigned_transaction(call.into()) {
            Ok(()) => {
                debug!(target: "runtime::kumandra", "Submitted Kumandra vote");
            }
            Err(()) => {
                error!(target: "runtime::kumandra", "Error submitting Kumandra vote");
            }
        }
    }
}
...
impl<T: Config> Pallet<T> {
    fn validate_root_block(
        ...
        ValidTransaction::with_tag_prefix("KumandraRootBlock")
    ...
    fn validate_vote(
        ...
        ValidTransaction::with_tag_prefix("KumandraVote")
...
fn check_vote<T: Config>(
    ...
    if is_equivocating {
        ...
        return Err(CheckVoteError::Equivocated(KumandraEquivocationOffence {
            slot,
            offender: public_key,
        }));
    }
...
impl<T: Config> OnTimestampSet<T::Moment> for Pallet<T> {
    fn on_timestamp_set(moment: T::Moment) {
        ...
        assert!(
            ...
            "Kumandra slot duration cannot be zero."
        );
...
impl<T: Config> kumandra_runtime_primitives::FindVotingRewardAddresses<T::AccountId> for Pallet<T> {

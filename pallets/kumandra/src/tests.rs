...
//! Consensus extension module tests for Kumandra consensus.
...
use crate::mock::{
    ...
    GlobalRandomnessUpdateInterval, Origin, ReportLongevity, Kumandra, System, Test,
...
use crate::{
    ...
    ParentBlockVoters, RecordsRoot, KumandraEquivocationOffence, WeightInfo,
};
use codec::Encode;
...
fn genesis_slot_is_correct() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(*Kumandra::genesis_slot(), 6);
    })
}
...
fn can_update_global_randomness() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Kumandra::global_randomnesses(), initial_randomnesses);
        ...
        let updated_randomnesses = Kumandra::global_randomnesses();
...
fn can_update_solution_range_on_era_change() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Kumandra::solution_ranges(), initial_solution_ranges);
        // enable solution range adjustment
        assert_ok!(Kumandra::enable_solution_range_adjustment(
            Origin::root(),
            None,
            None
        ));
        ...
        let updated_solution_ranges = Kumandra::solution_ranges();
        assert_eq!(
        ...
        let last_solution_range = Kumandra::solution_ranges().current;
        ...
        go_to_block(
            &keypair,
            8,
            u64::from(Kumandra::current_slot())
        ...
        assert!(Kumandra::solution_ranges().next.unwrap() > last_solution_range);
    })
}
...
fn solution_range_should_not_update_when_disabled() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Kumandra::solution_ranges().next.unwrap(),
            INITIAL_SOLUTION_RANGE
        );
    })
}
...
fn can_update_salt_on_eon_change() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Kumandra::salts(), initial_salts);
        ...
        let revealed_salts = Kumandra::salts();
        ...
        assert_eq!(Kumandra::salts(), revealed_salts);
...
fn report_equivocation_current_session_works() {
    new_test_ext().execute_with(|| {
        ...
        assert!(!Kumandra::is_in_block_list(&farmer_public_key));

        // report the equivocation
        Kumandra::report_equivocation(Origin::none(), Box::new(equivocation_proof)).unwrap();
...
fn report_equivocation_invalid_equivocation_proof() {
    new_test_ext().execute_with(|| {
        ...
        let assert_invalid_equivocation = |equivocation_proof| {
            assert_err!(
                Kumandra::report_equivocation(Origin::none(), Box::new(equivocation_proof),),
...
fn report_equivocation_validate_unsigned_prevents_duplicates() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            <Kumandra as sp_runtime::traits::ValidateUnsigned>::validate_unsigned(
        ...
        assert_eq!(
            ...
            Ok(ValidTransaction {
                ...
                requires: vec![],
                provides: vec![("KumandraEquivocation", tx_tag).encode()],
        ...
        assert_ok!(<Kumandra as sp_runtime::traits::ValidateUnsigned>::pre_dispatch(&inner));
        ...
        assert_err!(
            <Kumandra as sp_runtime::traits::ValidateUnsigned>::pre_dispatch(&inner),
            InvalidTransaction::Stale,
        );
    });
}
...
fn valid_equivocation_reports_dont_pay_fees() {
    new_test_ext().execute_with(|| {
        ...
        // report the equivocation.
        let post_info =
            Kumandra::report_equivocation(Origin::none(), Box::new(equivocation_proof.clone()))
                .unwrap();
        ...
        // duplicate.
        let post_info = Kumandra::report_equivocation(Origin::none(), Box::new(equivocation_proof))
            .err()
            .unwrap()
            .post_info;
...
fn store_root_block_works() {
    new_test_ext().execute_with(|| {
        ...
        Kumandra::store_root_blocks(Origin::none(), vec![root_block]).unwrap();
        assert_eq!(
            System::events(),
            vec![EventRecord {
                phase: Phase::Initialization,
                event: Event::Kumandra(crate::Event::RootBlockStored { root_block }),
                topics: vec![],
            }]
        );
    });
...
fn store_root_block_validate_unsigned_prevents_duplicates() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            <Kumandra as sp_runtime::traits::ValidateUnsigned>::pre_dispatch(&inner2),
            InvalidTransaction::BadMandatory,
        );
    });
}
...
fn vote_block_listed() {
    new_test_ext().execute_with(|| {
        ...
        let signed_vote = create_signed_vote(
            ...
            Kumandra::current_slot() + 1,
            &Kumandra::global_randomnesses().current,
            Kumandra::salts().current,
            piece,
            1,
        );
...
fn vote_randomness_update() {
    new_test_ext().execute_with(|| {
        ...
        let signed_vote = create_signed_vote(
            ...
            Kumandra::current_slot() + 1,
            &Kumandra::global_randomnesses().next.unwrap(),
            {
...
fn vote_salt_update() {
    new_test_ext().execute_with(|| {
        ...
        go_to_block(
            ...
            u64::from(
                (u64::from(Kumandra::current_slot()) as u32 / EonDuration::get() + 1)
                    * EonDuration::get(),
            ),
            1,
        );
...
fn vote_equivocation_current_block_plus_vote() {
    new_test_ext().execute_with(|| {
        ...
        let slot = Kumandra::current_slot() + 1;
        ...
        assert_err!(
            super::check_vote::<Test>(&signed_vote, false),
            CheckVoteError::Equivocated(KumandraEquivocationOffence {
                slot,
...
fn vote_equivocation_parent_block_plus_vote() {
    new_test_ext().execute_with(|| {
        ...
        let slot = Kumandra::current_slot();
        ...
        Kumandra::pre_dispatch_vote(&signed_vote).unwrap();

        assert_err!(
            Kumandra::vote(Origin::none(), Box::new(signed_vote)),
            DispatchError::Other("Equivocated"),
        );

        // Block author doesn't get reward after equivocation
        assert_matches!(Kumandra::find_block_reward_address(), None);
    });
}
...
fn vote_equivocation_current_voters_duplicate() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(Kumandra::find_voting_reward_addresses().len(), 0);
    });
}
...
fn enabling_block_rewards_works() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_matches!(Kumandra::find_block_reward_address(), Some(1));
        assert_eq!(Kumandra::find_voting_reward_addresses(), vec![2]);
...
fn allow_authoring_by_anyone_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_err!(
            Kumandra::enable_authoring_by_anyone(Origin::signed(1)),
            DispatchError::BadOrigin
        );
        Kumandra::enable_authoring_by_anyone(Origin::root()).unwrap();

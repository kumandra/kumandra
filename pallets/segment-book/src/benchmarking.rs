...
fn intent_one<T: Config>() {
	...
	<ParamSetA<T>>::insert(&caller, ParamInfo { peer_id: peerid, segment_id, rand: random });
}
...
fn intent_two<T: Config>() {
	...
	let silce_info =
		FileSilceInfo { peer_id: peerid, segment_id, uncid, rand: random, hash, shardhash };
	if <MinerHoldSlice<T>>::contains_key(&caller) {
		...
		<MinerHoldSlice<T>>::insert(&caller, value)
	}
}
...
fn intent_po_st<T: Config>() {
	...
	<ParamSetB<T>>::insert(&caller, ParamInfo { peer_id, segment_id, rand: random });
	...
	<ParamSetD<T>>::insert(&caller, ParamInfo { peer_id, segment_id, rand: random });
}

fn add_block_b<T: Config>() {
	let caller: T::AccountId = whitelisted_caller();
	<BlockNumberB<T>>::insert(&caller, PeerFileNum { block_num: 0u128.into(), total_num: 1 });
}

fn add_block_d<T: Config>() {
	let caller: T::AccountId = whitelisted_caller();
	<BlockNumberD<T>>::insert(&caller, PeerFileNum { block_num: 0u128.into(), total_num: 1 });
}

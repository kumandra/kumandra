...
use sp_std::prelude::*;
use kp_scheduler_credit::SchedulerCreditCounter;
...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + pallet_kumandra_staking::Config {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn registration_scheduler(
			...
			let acc = <pallet_kumandra_staking::Pallet<T>>::bonded(&stash_account)
	...
	fn punish_scheduler(acc: <T as frame_system::Config>::AccountId) {
		...
		for v in scheduler_list {
			if v.controller_user == acc {
				pallet_kumandra_staking::slashing::slash_scheduler::<T>(&v.stash_user);

...
use kp_consensus::offence::{OffenceDetails, OffenceError, ReportOffence};
...
fn should_report_an_farmer_and_trigger_on_offence() {
    new_test_ext().execute_with(|| {
        ...
        // when
        OffencesKumandra::report_offence(offence).unwrap();
...
fn should_not_report_the_same_farmer_twice_in_the_same_slot() {
    new_test_ext().execute_with(|| {
        ...
        OffencesKumandra::report_offence(offence.clone()).unwrap();
        with_on_offence_fractions(|f| {
            assert_eq!(f.clone(), vec![Perbill::from_percent(25)]);
            f.clear();
        });
        ...
        assert_eq!(
            OffencesKumandra::report_offence(offence),
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            vec![EventRecord {
                ...
                event: Event::OffencesKumandra(crate::Event::Offence {
                    kind: KIND,
...
fn reports_if_an_offence_is_dup() {
    ...
    new_test_ext().execute_with(|| {
        ...
        // offence
        assert!(
            !<OffencesKumandra as ReportOffence<_, TestOffence>>::is_known_offence(
        ...
        OffencesKumandra::report_offence(test_offence.clone()).unwrap();
        ...
        assert_eq!(
            OffencesKumandra::report_offence(test_offence.clone()),
            Err(OffenceError::DuplicateReport)
        );
...
fn should_properly_count_offences() {
    ...
    new_test_ext().execute_with(|| {
        ...
        OffencesKumandra::report_offence(offence1).unwrap();
        with_on_offence_fractions(|f| {
            assert_eq!(f.clone(), vec![Perbill::from_percent(25)]);
            f.clear();
        });
        ...
        // when
        // report for the second time
        OffencesKumandra::report_offence(offence2).unwrap();
...
fn should_properly_sort_offences() {
    new_test_ext().execute_with(|| {
        ...
        // report for the second time
        OffencesKumandra::report_offence(offence2).unwrap();
        OffencesKumandra::report_offence(offence3).unwrap();
        OffencesKumandra::report_offence(offence4).unwrap();

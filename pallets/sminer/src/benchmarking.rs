...
fn add_calculate<T: Config>() {
	...
	let order: Vec<CalculateRewardOrder<T>> =
		vec![CalculateRewardOrder::<T> { calculate_reward: reward, start_t: now, deadline }];
	<CalculateRewardOrderMap<T>>::insert(caller, order);
}

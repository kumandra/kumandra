...
fn register_scheduler(stash: AccountId, controller: AccountId) -> DispatchResult {
    pallet_kumandra_staking::Bonded::<Test>::insert(&stash, controller.clone());
...
fn upload_declaration() {
    new_test_ext().execute_with(|| {
        let acc1 = mock::account1();
        let file_name = "kumandra-book".as_bytes().to_vec();
        let file_hash = "kumandra056237k7k439902190502".as_bytes().to_vec();
...
fn upload_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(upload_declaration_alias(acc1, "kumandra-book".as_bytes().to_vec(), mfi.file_hash.to_vec()));

...
pub trait Config:
	crate::Config + pallet_kumandra_staking::Config + pallet_file_map::Config + pallet_sminer::Config
{
}
...
pub fn add_scheduler<T: Config>() -> Result<T::AccountId, &'static str> {
	...
	let amount = <T as pallet_kumandra_staking::Config>::Currency::minimum_balance() * 10u32.into();
...
benchmarks! {
	...
	upload_declaration {
		...
		let file_hash = "kumandra-13540202190502".as_bytes().to_vec();
		let file_name = "test-file".as_bytes().to_vec();
	...
	delete_file {
		let file_hash = "kumandra_05020219520".as_bytes().to_vec();

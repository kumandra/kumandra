...
use kp_consensus::offence::{OffenceDetails, OnOffenceHandler};
use kp_consensus::{EquivocationProof, FarmerPublicKey};
use sp_io::hashing;
...
mod pallet {
    ...
    use kp_consensus::{EquivocationProof, FarmerPublicKey};
    use sp_std::prelude::*;
    use kumandra_core_primitives::{RootBlock, Sha256Hash};
    ...
    #[pallet::storage]
    pub(super) type PorRandomness<T> = StorageValue<_, kumandra_core_primitives::Randomness>;
...
impl<T: Config> Pallet<T> {
    ...
    fn do_report_equivocation(
        ...
        if !kp_consensus::check_equivocation_proof(equivocation_proof) {
    ...
    fn derive_next_salt_from_randomness(
        eon_index: u64,
        randomness: &kumandra_core_primitives::Randomness,
...
impl<T: Config> frame_support::traits::FindAuthor<T::AccountId> for Pallet<T> {
        ...
        I: 'a + IntoIterator<Item = (ConsensusEngineId, &'a [u8])>,
    {
        digests
            .into_iter()
            .find_map(|(id, data)| DigestItemRef::PreRuntime(&id, data).as_kumandra_pre_digest())

...
use kc_consensus::digests::CompatibleDigestItem;
use kc_consensus::{
...
/// The smallest unit of the token is called KTIC.
pub const KTIC: Balance = 1;
/// Kumandra Credits have 18 decimal places.
pub const DECIMAL_PLACES: u8 = 18;
/// One Kumandra Credit.
pub const KMD: Balance = (10 * KTIC).pow(DECIMAL_PLACES as u32);
...
impl Get<u128> for TotalSpacePledged {
    fn get() -> u128 {
        ...
        u128::from(u64::MAX)
            ...
            / u128::from(Kumandra::solution_ranges().current)
...
impl Get<u128> for BlockchainHistorySize {
    fn get() -> u128 {
        u128::from(Kumandra::archived_history_size())
    }
}
...
impl pallet_rewards::Config for Runtime {
    ...
    type FindBlockRewardAddress = Kumandra;
    type FindVotingRewardAddresses = Kumandra;
    type WeightInfo = ();
}
...
impl_runtime_apis! {
    ...
    impl kc_consensus::KumandraApi<Block, FarmerPublicKey> for Runtime {
        ...
        fn submit_vote_extrinsic(
            ...
            Kumandra::submit_vote(SignedVote {
                vote: Vote::V0 {
                    height,
                    parent_hash,
                    slot,

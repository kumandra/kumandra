...
pub fn wait_for_event<C: AnyConnection, E: Decode + Clone, P: Fn(E) -> bool>(
    ...
    info!(target: "kumandra-client", "Creating event subscription {}/{}", module, variant);
    ...
    loop {
        ...
        match args {
            ...
            Ok(_) => (),
            Err(why) => error!(target: "kumandra-client", "Error {:?}", why),
        }
    }
}
...
pub fn wait_for_finalized_block<C: AnyConnection>(
    ...
    {
        info!(target: "kumandra-client", "Received header for a block number {:?}", header.number);

...
impl From<Vec<u8>> for Keys {
    fn from(bytes: Vec<u8>) -> Self {
        ...
        Self {
            ...
            kumandra: bytes[32..64].try_into().unwrap(),
        }
    }
}
...
pub fn change_validators(
    ...
    info!(target: "kumandra-client", "New validators: reserved: {:#?}, non_reserved: {:#?}, validators_per_session: {:?}", new_reserved_validators, new_non_reserved_validators, validators_per_session);
...
pub fn wait_for<C: AnyConnection>(
    ...
    info!(target: "kumandra-client", "Waiting for session {}", session_index);
    ...
    wait_for_event(
        ...
        ("Session", "NewSession"),
        |e: NewSessionEvent| {
            info!(target: "kumandra-client", "New session {}", e.session_index);

...
//! Schema for Kumandra block weight in the aux-db.
    ...
    T: Decode,
{
    let corrupt = |e: codec::Error| {
        ClientError::Backend(format!("Kumandra DB is corrupted. Decode error: {}", e))
    };
...
pub(crate) fn write_block_weight<H: Encode, F, R>(
    block_hash: H,
    block_weight: KumandraBlockWeight,
    write_aux: F,
) -> R
where
...
pub(crate) fn load_block_weight<H: Encode, B: AuxStore>(
    backend: &B,
    block_hash: H,
) -> ClientResult<Option<KumandraBlockWeight>> {

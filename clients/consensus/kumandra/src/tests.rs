...
use crate::{
    find_pre_digest, start_kumandra, Config, NewSlotNotification, KumandraLink, KumandraParams,
    KumandraVerifier,
};
use codec::Encode;
...
use kp_consensus::inherents::InherentDataProvider;
...
use kumandra_core_primitives::{FlatPieces, LocalChallenge, Piece, Solution, Tag, TagSignature};
...
impl CreateInherentDataProviders<TestBlock, KumandraLink<TestBlock>>
    for TestCreateInherentDataProviders
{
    ...
    async fn create_inherent_data_providers(
        ...
        extra_args: KumandraLink<TestBlock>,
...
type KumandraBlockImport = PanickingBlockImport<
    crate::KumandraBlockImport<
        TestBlock,
        TestClient,
        Arc<TestClient>,
        AlwaysCanAuthor,
...
impl DummyProposer {
    fn propose_with(
        &mut self,
        pre_digests: Digest,
    ) -> future::Ready<Result<KumandraProposal, sp_blockchain::Error>> {
...
impl Proposer<TestBlock> for DummyProposer {
    ...
    type Proposal = future::Ready<Result<KumandraProposal, Self::Error>>;
...
type KumandraPeer = Peer<Option<PeerData>, KumandraBlockImport>;

pub struct KumandraTestNet {
    peers: Vec<KumandraPeer>,
}
...
impl TestNetFactory for KumandraTestNet {
    type Verifier = TestVerifier;
    type PeerData = Option<PeerData>;
    type BlockImport = KumandraBlockImport;

    /// Create new test network with peers and given config.
    fn from_config(_config: &ProtocolConfig) -> Self {
        debug!(target: "kumandra", "Creating test network from config");
        KumandraTestNet { peers: Vec::new() }
    }
    ...
    fn make_verifier(
        ...
        let client = client.as_client();
        trace!(target: "kumandra", "Creating a verifier");
    ...
    fn peer(&mut self, i: usize) -> &mut KumandraPeer {
        trace!(target: "kumandra", "Retrieving a peer");
        &mut self.peers[i]
    }

    fn peers(&self) -> &Vec<KumandraPeer> {
        trace!(target: "kumandra", "Retrieving peers");
        &self.peers
    }

    fn mut_peers<F: FnOnce(&mut Vec<KumandraPeer>)>(&mut self, closure: F) {
...
fn rejects_empty_block() {
    ...
    let mut net = KumandraTestNet::new(3);
...
fn run_one_test(mutator: impl Fn(&mut TestHeader, Stage) + Send + Sync + 'static) {
    ...
    let net = KumandraTestNet::new(3);

    let net = Arc::new(Mutex::new(net));
    let mut import_notifications = Vec::new();
    let mut kumandra_futures = Vec::<Pin<Box<dyn Future<Output = ()>>>>::new();
    ...
    for peer_id in [0, 1, 2_usize].iter() {
        ...
        let data = peer
            .data
            .as_ref()
            .expect("Kumandra link set up during initialization");
        ...
        let kumandra_worker = start_kumandra(KumandraParams {
            block_import: data
                .block_import
                .lock()
                .take()
        ...
        kumandra_futures.push(Box::pin(kumandra_worker));
        kumandra_futures.push(Box::pin(kumandra_farmer));
    }
    tokio_runtime.block_on(future::select(
        ...
        future::select(
            ...
            future::join_all(kumandra_futures),
        ),
    ));
}
        ...
        header.digest_mut().logs = v
            ...
            .filter(|v| {
                stage == Stage::PostSeal
                    || DigestItem::as_kumandra_pre_digest::<FarmerPublicKey>(v).is_none()
            })
            .collect()
    })
}
...
fn rejects_missing_seals() {
    run_one_test(|header: &mut TestHeader, stage| {
        ...
        header.digest_mut().logs = v
            .into_iter()
            .filter(|v| stage == Stage::PreSeal || DigestItem::as_kumandra_seal(v).is_none())
            .collect()
    })
}
...
fn wrong_consensus_engine_id_rejected() {
    ...
    assert!(CompatibleDigestItem::as_kumandra_pre_digest::<FarmerPublicKey>(&bad_seal).is_none());
    assert!(CompatibleDigestItem::as_kumandra_seal(&bad_seal).is_none())
}

#[test]
fn malformed_pre_digest_rejected() {
    sp_tracing::try_init_simple();
    let bad_seal = DigestItem::kumandra_seal(FarmerSignature::unchecked_from([0u8; 64]));
    assert!(CompatibleDigestItem::as_kumandra_pre_digest::<FarmerPublicKey>(&bad_seal).is_none());
}
...
fn sig_is_not_pre_digest() {
    ...
    let keypair = Keypair::generate();
    let ctx = schnorrkel::context::signing_context(REWARD_SIGNING_CONTEXT);
    let bad_seal = DigestItem::kumandra_seal(FarmerSignature::unchecked_from(
        keypair.sign(ctx.bytes(b"")).to_bytes(),
    ));
    assert!(CompatibleDigestItem::as_kumandra_pre_digest::<FarmerPublicKey>(&bad_seal).is_none());
    assert!(CompatibleDigestItem::as_kumandra_seal(&bad_seal).is_some())
}
...
fn can_author_block() {
    ...
    loop {
        match dummy_claim_slot(i.into()) {
            None => i += 1,
            Some(s) => {
                debug!(target: "kumandra", "Authored block {:?}", s.0);
                break;
            }
        }
    }
...
// Propose and import a new Kumandra block on top of the given parent.
fn propose_and_import_block<Transaction: Send + 'static>(
    ...
    let (pre_digest, signature) = {
        ...
        (
            sp_runtime::generic::Digest {
                logs: vec![DigestItem::kumandra_pre_digest(&PreDigest {
                    slot,
                    solution: Solution {
                        public_key: FarmerPublicKey::unchecked_from(keypair.public.to_bytes()),
    ...
    let seal = DigestItem::kumandra_seal(signature.to_vec().try_into().unwrap());
...
fn verify_slots_are_strictly_increasing() {
    let mut net = KumandraTestNet::new(1);
...
//     let task_manager = TaskManager::new(tokio_runtime.handle().clone(), None).unwrap();
//
//     super::start_kumandra_archiver(&data.link, client.clone(), &task_manager.spawn_essential_handle());
//

...
use kp_consensus::verification::{CheckedHeader, VerificationError};
use kp_consensus::{verification, FarmerPublicKey, FarmerSignature, KumandraApi};
...
use kumandra_core_primitives::{BlockNumber, RootBlock, Salt, Sha256Hash, Solution};
use kumandra_solving::{derive_global_challenge, derive_target, REWARD_SIGNING_CONTEXT};

// TODO: Hack for Kumandra Testnet 1b launch.
const KTESTNET_1B_GENESIS_HASH: &[u8] = &[
    ...
    SN: Fn() -> Slot + Send + Sync + 'static,
{
    ...
    > {
        ...
        // TODO: Hack for Kumandra Testnet 1b launch. These blocks should have correct block author.
        if *block.header.number() <= 33_581_u32.into()
            && self.client.info().genesis_hash.as_ref() == KTESTNET_1B_GENESIS_HASH
        ...
        {
            return Err(
                "Unexpected block author during early history of Kumandra Testnet 1b, please ignore this \
                message"
                    .to_string(),
            );
        }
        ...
        let checked_header = {
            ...
            // TODO: Hack for Kumandra Testnet 1b launch. Solution range should have been updated already.
    ...
    CIDP: CreateInherentDataProviders<Block, KumandraLink<Block>> + Send + Sync + 'static,
{
    ...
    async fn import_block(
        ...
        let added_weight = {
            ...
            u128::from(u64::MAX - kumandra_core_primitives::bidirectional_distance(&target, &tag))
        };

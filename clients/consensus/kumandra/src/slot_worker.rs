...
use kp_consensus::{FarmerPublicKey, FarmerSignature, SignedVote, KumandraApi, Vote};
...
use kumandra_solving::{derive_global_challenge, derive_target};

pub(super) struct KumandraSlotWorker<Block: BlockT, Client, E, I, SO, L, BS> {
    ...
    pub(super) kumandra_link: KumandraLink<Block>,
...
impl<Block, Client, E, I, Error, SO, L, BS> SimpleSlotWorker<Block>
    for KumandraSlotWorker<Block, Client, E, I, SO, L, BS>
where
    Block: BlockT,
    ...
    Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
    ...
    async fn claim_slot(
        ...
        // TODO: Hack for Kumandra Testnet 1b launch. These blocks are already produced, avoid claiming them.
        if *parent_header.number() <= 33_671_u32.into()
            && self.client.info().genesis_hash.as_ref() == crate::KTESTNET_1B_GENESIS_HASH
        {
            return None;
        }

        debug!(target: "kumandra", "Attempting to claim slot {}", slot);
        ...
        let (solution_sender, mut solution_receiver) =
            tracing_unbounded("kumandra_slot_solution_stream");
        ...
        while let Some(solution) = solution_receiver.next().await {
            ...
            if let Err(error) = solution_verification_result {
                warn!(target: "kumandra", "Invalid solution received for slot {slot}: {error:?}");
            } else {
                ...
                {
                    info!(target: "kumandra", "🚜 Claimed block at slot {slot}");
                    ...
                    info!(target: "kumandra", "🗳️ Claimed vote at slot {slot}");
    ...
    fn pre_digest_data(&self, _slot: Slot, claim: &Self::Claim) -> Vec<DigestItem> {
        vec![DigestItem::kumandra_pre_digest(claim)]
    }
    ...
    async fn block_import_params(
        ...
        let digest_item = DigestItem::kumandra_seal(signature);
...
impl<Block, Client, E, I, Error, SO, L, BS> KumandraSlotWorker<Block, Client, E, I, SO, L, BS>
where
    Block: BlockT,
    Client: ProvideRuntimeApi<Block>
        + HeaderBackend<Block>
    ...
    Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
    ...
    async fn sign_reward(
        ...
        let (signature_sender, mut signature_receiver) =
            tracing_unbounded("kumandra_signature_signing_stream");

...
use kp_consensus::digests::{
    extract_pre_digest, extract_kumandra_digest_items, Error as DigestError, KumandraDigestItems,
};
use kp_consensus::{
    check_header, CheckedHeader, FarmerPublicKey, FarmerSignature, KumandraApi, VerificationError,
    VerificationParams,
};
...
use kumandra_solving::{derive_global_challenge, derive_target, REWARD_SIGNING_CONTEXT};
use kumandra_verification::{Error as VerificationPrimitiveError, VerifySolutionParams};
...
pub struct KumandraLink<Block: BlockT> {
    ...
    reward_signing_notification_sender: KumandraNotificationSender<RewardSigningNotification>,
    ...
    imported_block_notification_stream:
        KumandraNotificationStream<ImportedBlockNotification<Block>>,
...
impl<Block: BlockT> KumandraLink<Block> {
    ...
    pub fn reward_signing_notification_stream(
        &self,
    ) -> KumandraNotificationStream<RewardSigningNotification> {
...
where
    ...
    Client::Api: BlockBuilderApi<Block> + KumandraApi<Block, FarmerPublicKey>,
    ...
    SN: Fn() -> Slot + Send + Sync + 'static,
{
    ...
    > {
        ...
        let kumandra_digest_items = extract_kumandra_digest_items::<
            Block::Header,
            FarmerPublicKey,
            FarmerPublicKey,
            FarmerSignature,
        ...
        let pre_digest = kumandra_digest_items.pre_digest;
        ...
        let checked_header = {
            ...
            if *block.header.number() >= 33_672_u32.into()
                && self.client.info().genesis_hash.as_ref() == GEMINI_1B_GENESIS_HASH
                && kumandra_digest_items.solution_range == 12_009_599_006_321_322_u64
            {
                debug!(
                    target: "kumandra",
            ...
            check_header::<_, FarmerPublicKey>(
                VerificationParams {
                    ...
                    verify_solution_params: VerifySolutionParams {
                        global_randomness: &kumandra_digest_items.global_randomness,
                        solution_range: kumandra_digest_items.solution_range,
                        salt: kumandra_digest_items.salt,
...
pub struct KumandraBlockImport<Block: BlockT, Client, I, CAW, CIDP> {
    ...
    imported_block_notification_sender:
        KumandraNotificationSender<ImportedBlockNotification<Block>>,
...
where
    ...
    Client::Api: BlockBuilderApi<Block> + KumandraApi<Block, FarmerPublicKey> + ApiExt<Block>,
    CAW: CanAuthorWith<Block> + Send + Sync + 'static,
    CIDP: CreateInherentDataProviders<Block, KumandraLink<Block>> + Send + Sync + 'static,
{
    ...
    async fn block_import_verification(
        ...
        kumandra_digest_items: &KumandraDigestItems<
            FarmerPublicKey,
            FarmerPublicKey,
            FarmerSignature,
        >,
        ...
        let (correct_global_randomness, correct_solution_range, correct_salt) =
            if header.number().is_one() {
                ...
                let parent_kumandra_digest_items = extract_kumandra_digest_items::<
                    _,
                    FarmerPublicKey,
                    FarmerPublicKey,
                    FarmerSignature,
                ...
                let correct_global_randomness =
                    match parent_kumandra_digest_items.next_global_randomness {
                        ...
                        None => {
                            ...
                            if is_gemini_1b {
                                ...
                                parent_kumandra_digest_items.global_randomness
                            }
                        }
                    };

                let correct_solution_range = match parent_kumandra_digest_items.next_solution_range
                {
                    ...
                    None => {
                        ...
                        if is_gemini_1b {
                            ...
                            parent_kumandra_digest_items.solution_range
                        }
                    }
                };

                let correct_salt = match parent_kumandra_digest_items.next_salt {
                    Some(salt) => salt,
                    None => {
                        // TODO: Remove `if` branch when breaking protocol
                        if is_gemini_1b {
                            ...
                            parent_kumandra_digest_items.salt
                        }
                    }
                };
        ...
        if kumandra_digest_items.global_randomness != correct_global_randomness {
            return Err(Error::InvalidGlobalRandomness(block_hash));
        }

        if kumandra_digest_items.solution_range != correct_solution_range {
            return Err(Error::InvalidSolutionRange(block_hash));
        }

        if kumandra_digest_items.salt != correct_salt {
        ...
        kumandra_verification::check_piece(
            records_root,
            position,
            RECORD_SIZE,
            &pre_digest.solution,
    ...
    CIDP: CreateInherentDataProviders<Block, KumandraLink<Block>> + Send + Sync + 'static,
{
    ...
    async fn import_block(
        ...
        let kumandra_digest_items = extract_kumandra_digest_items(&block.header)
        ...
        let added_weight = {
            let global_challenge = derive_global_challenge(
                &kumandra_digest_items.global_randomness,
        ...
        for (&segment_index, records_root) in &kumandra_digest_items.records_roots {
    ...
    CIDP: CreateInherentDataProviders<Block, KumandraLink<Block>> + Send + Sync + 'static,
{
    ...
    let (reward_signing_notification_sender, reward_signing_notification_stream) =
        notification::channel("kumandra_reward_signing_notification_stream");

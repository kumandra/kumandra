...
if $BUILD_KUMANDRA_NODE ; then
  cargo build --release -p kumandra-node --features "short_session"
fi

declare -a account_ids
for i in $(seq 0 "$(( N_VALIDATORS + N_NON_VALIDATORS - 1 ))"); do
  account_ids+=($(./target/release/kumandra-node key inspect "//$i" | grep "SS58 Address:" | awk '{print $3;}'))
done
...
./target/release/kumandra-node bootstrap-chain --base-path "$BASE_PATH" --account-ids "$validator_ids_string" --chain-type local > "$BASE_PATH/chainspec.json"

for i in $(seq "$N_VALIDATORS" "$(( N_VALIDATORS + N_NON_VALIDATORS - 1 ))"); do
  echo "Bootstrapping node $i"
  account_id=${account_ids[$i]}
  ./target/release/kumandra-node bootstrap-node --base-path "$BASE_PATH" --account-id "$account_id" --chain-type local
done

addresses=()
for i in $(seq 0 "$(( N_VALIDATORS + N_NON_VALIDATORS - 1 ))"); do
    pk=$(./target/release/kumandra-node key inspect-node-key --file $BASE_PATH/${account_ids[$i]}/p2p_secret)

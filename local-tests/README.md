...
3. `test_update.py` - a script simulating an update of `kumandra-node` binary together with updating chain's runtime. It requires two different `kumandra-node` binaries (pre-update and post-update) and a compiled wasm runtime of the post-update binary. Please check the top part to see how to set up env variables with correct paths before running.

...
# Path to the pre-update kumandra-node binary:
oldbin = abspath(os.getenv('OLD_BINARY', join(workdir, 'kumandra-node-old')))
# Path to the post-update kumandra-node binary:
newbin = abspath(os.getenv('NEW_BINARY', join(workdir, 'kumandra-node-new')))
# Path to the post-update compiled runtime:
runtime = abspath(os.getenv('NEW_RUNTIME', join(workdir, 'kumandra_runtime.compact.wasm')))
# Path to dragon:
DRAGON = abspath('../bin/dragon/target/release/dragon')
...
subprocess.check_call(
    [DRAGON, '--node', 'localhost:9945', '--seed', phrases[0],
